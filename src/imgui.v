...
fn im_gui_create_context(shared_font_atlas &ImFontAtlas) &ImGuiContext

@[c:'ImGui_DestroyContext']
fn im_gui_destroy_context(ctx &ImGuiContext)

// NULL = destroy current context
@[c:'ImGui_GetCurrentContext']
fn im_gui_get_current_context() &ImGuiContext

@[c:'ImGui_SetCurrentContext']
fn im_gui_set_current_context(ctx &ImGuiContext)

// Main
@[c:'ImGui_GetIO']
fn im_gui_get_io() &ImGuiIO

// access the ImGuiIO structure (mouse/keyboard/gamepad inputs, time, various configuration options/flags)
@[c:'ImGui_GetPlatformIO']
fn im_gui_get_platform_io() &ImGuiPlatformIO

// access the ImGuiPlatformIO structure (mostly hooks/functions to connect to platform/renderer and OS Clipboard, IME etc.)
@[c:'ImGui_GetStyle']
fn im_gui_get_style() &ImGuiStyle
...
@[c:'ImGui_GetDrawData']
fn im_gui_get_draw_data() &ImDrawData

// valid after Render() and until the next call to NewFrame(). this is what you have to render.
// Demo, Debug, Information
@[c:'ImGui_ShowDemoWindow']
fn im_gui_show_demo_window(p_open &bool)

// create Demo window. demonstrate most ImGui features. call this to learn about the library! try to make it always available in your application!
@[c:'ImGui_ShowMetricsWindow']
fn im_gui_show_metrics_window(p_open &bool)

// create Metrics/Debugger window. display Dear ImGui internals: windows, draw commands, various internal state, etc.
@[c:'ImGui_ShowDebugLogWindow']
fn im_gui_show_debug_log_window(p_open &bool)

// create Debug Log window. display a simplified log of important dear imgui events.
@[c:'ImGui_ShowIDStackToolWindow']
fn im_gui_show_ids_tack_tool_window()

// Implied p_open = NULL
@[c:'ImGui_ShowIDStackToolWindowEx']
fn im_gui_show_ids_tack_tool_window_ex(p_open &bool)

// create Stack Tool window. hover items with mouse to query information about the source of their unique ID.
@[c:'ImGui_ShowAboutWindow']
fn im_gui_show_about_window(p_open &bool)

// create About window. display Dear ImGui version, credits and build/system information.
@[c:'ImGui_ShowStyleEditor']
fn im_gui_show_style_editor(ref &ImGuiStyle)

// add style editor block (not a window). you can pass in a reference ImGuiStyle structure to compare to, revert to and save to (else it uses the default style)
@[c:'ImGui_ShowStyleSelector']
fn im_gui_show_style_selector(label &i8) bool

// add style selector block (not a window), essentially a combo listing the default styles.
@[c:'ImGui_ShowFontSelector']
fn im_gui_show_font_selector(label &i8)

// add font selector block (not a window), essentially a combo listing the loaded fonts.
@[c:'ImGui_ShowUserGuide']
fn im_gui_show_user_guide()

// add basic help/info block (not a window): how to manipulate ImGui as an end-user (mouse/keyboard controls).
@[c:'ImGui_GetVersion']
fn im_gui_get_version() &i8

// get the compiled version string e.g. "1.80 WIP" (essentially the value for IMGUI_VERSION from the compiled version of imgui.cpp)
// Styles
@[c:'ImGui_StyleColorsDark']
fn im_gui_style_colors_dark(dst &ImGuiStyle)

// new, recommended style (default)
@[c:'ImGui_StyleColorsLight']
fn im_gui_style_colors_light(dst &ImGuiStyle)

// best used with borders and a custom, thicker font
@[c:'ImGui_StyleColorsClassic']
fn im_gui_style_colors_classic(dst &ImGuiStyle)
...
@[c:'ImGui_Begin']
fn im_gui_begin(name &i8, p_open &bool, flags ImGuiWindowFlags) bool
...
//    BeginXXX function returned true. Begin and BeginChild are the only odd ones out. Will be fixed in a future update.]
@[c:'ImGui_BeginChild']
fn im_gui_begin_child(str_id &i8, size ImVec2, child_flags ImGuiChildFlags, window_flags ImGuiWindowFlags) bool

@[c:'ImGui_BeginChildID']
fn im_gui_begin_child_id(id ImGuiID, size ImVec2, child_flags ImGuiChildFlags, window_flags ImGuiWindowFlags) bool
...
fn im_gui_is_window_appearing() bool

@[c:'ImGui_IsWindowCollapsed']
fn im_gui_is_window_collapsed() bool

@[c:'ImGui_IsWindowFocused']
fn im_gui_is_window_focused(flags ImGuiFocusedFlags) bool

// is current window focused? or its root/child, depending on flags. see flags for options.
@[c:'ImGui_IsWindowHovered']
fn im_gui_is_window_hovered(flags ImGuiHoveredFlags) bool

// is current window hovered and hoverable (e.g. not blocked by a popup/modal)? See ImGuiHoveredFlags_ for options. IMPORTANT: If you are trying to check whether your mouse should be dispatched to Dear ImGui or to your underlying app, you should not use this function! Use the 'io.WantCaptureMouse' boolean for that! Refer to FAQ entry "How can I tell whether to dispatch mouse/keyboard to Dear ImGui or my application?" for details.
@[c:'ImGui_GetWindowDrawList']
fn im_gui_get_window_draw_list() &ImDrawList

// get draw list associated to the current window, to append your own drawing primitives
@[c:'ImGui_GetWindowPos']
fn im_gui_get_window_pos() ImVec2

// get current window position in screen space (IT IS UNLIKELY YOU EVER NEED TO USE THIS. Consider always using GetCursorScreenPos() and GetContentRegionAvail() instead)
@[c:'ImGui_GetWindowSize']
fn im_gui_get_window_size() ImVec2

// get current window size (IT IS UNLIKELY YOU EVER NEED TO USE THIS. Consider always using GetCursorScreenPos() and GetContentRegionAvail() instead)
@[c:'ImGui_GetWindowWidth']
fn im_gui_get_window_width() f32

// get current window width (IT IS UNLIKELY YOU EVER NEED TO USE THIS). Shortcut for GetWindowSize().x.
@[c:'ImGui_GetWindowHeight']
fn im_gui_get_window_height() f32

// get current window height (IT IS UNLIKELY YOU EVER NEED TO USE THIS). Shortcut for GetWindowSize().y.
// Window manipulation
// - Prefer using SetNextXXX functions (before Begin) rather that SetXXX functions (after Begin).
@[c:'ImGui_SetNextWindowPos']
fn im_gui_set_next_window_pos(pos ImVec2, cond ImGuiCond)

// Implied pivot = ImVec2(0, 0)
@[c:'ImGui_SetNextWindowPosEx']
fn im_gui_set_next_window_pos_ex(pos ImVec2, cond ImGuiCond, pivot ImVec2)

// set next window position. call before Begin(). use pivot=(0.5f,0.5f) to center on given point, etc.
@[c:'ImGui_SetNextWindowSize']
fn im_gui_set_next_window_size(size ImVec2, cond ImGuiCond)

// set next window size. set axis to 0.0f to force an auto-fit on this axis. call before Begin()
@[c:'ImGui_SetNextWindowSizeConstraints']
fn im_gui_set_next_window_size_constraints(size_min ImVec2, size_max ImVec2, custom_callback ImGuiSizeCallback, custom_callback_data voidptr)

// set next window size limits. use 0.0f or FLT_MAX if you don't want limits. Use -1 for both min and max of same axis to preserve current size (which itself is a constraint). Use callback to apply non-trivial programmatic constraints.
@[c:'ImGui_SetNextWindowContentSize']
fn im_gui_set_next_window_content_size(size ImVec2)

// set next window content size (~ scrollable client area, which enforce the range of scrollbars). Not including window decorations (title bar, menu bar, etc.) nor WindowPadding. set an axis to 0.0f to leave it automatic. call before Begin()
@[c:'ImGui_SetNextWindowCollapsed']
fn im_gui_set_next_window_collapsed(collapsed bool, cond ImGuiCond)

// set next window collapsed state. call before Begin()
@[c:'ImGui_SetNextWindowFocus']
fn im_gui_set_next_window_focus()

// set next window to be focused / top-most. call before Begin()
@[c:'ImGui_SetNextWindowScroll']
fn im_gui_set_next_window_scroll(scroll ImVec2)

// set next window scrolling value (use < 0.0f to not affect a given axis).
@[c:'ImGui_SetNextWindowBgAlpha']
fn im_gui_set_next_window_bg_alpha(alpha f32)

// set next window background color alpha. helper to easily override the Alpha component of ImGuiCol_WindowBg/ChildBg/PopupBg. you may also use ImGuiWindowFlags_NoBackground.
@[c:'ImGui_SetWindowPos']
fn im_gui_set_window_pos(pos ImVec2, cond ImGuiCond)

// (not recommended) set current window position - call within Begin()/End(). prefer using SetNextWindowPos(), as this may incur tearing and side-effects.
@[c:'ImGui_SetWindowSize']
fn im_gui_set_window_size(size ImVec2, cond ImGuiCond)

// (not recommended) set current window size - call within Begin()/End(). set to ImVec2(0, 0) to force an auto-fit. prefer using SetNextWindowSize(), as this may incur tearing and minor side-effects.
@[c:'ImGui_SetWindowCollapsed']
fn im_gui_set_window_collapsed(collapsed bool, cond ImGuiCond)

// (not recommended) set current window collapsed state. prefer using SetNextWindowCollapsed().
@[c:'ImGui_SetWindowFocus']
fn im_gui_set_window_focus()

// (not recommended) set current window to be focused / top-most. prefer using SetNextWindowFocus().
@[c:'ImGui_SetWindowFontScale']
fn im_gui_set_window_font_scale(scale f32)

// [OBSOLETE] set font scale. Adjust IO.FontGlobalScale if you want to scale all windows. This is an old API! For correct scaling, prefer to reload font + rebuild ImFontAtlas + call style.ScaleAllSizes().
@[c:'ImGui_SetWindowPosStr']
fn im_gui_set_window_pos_str(name &i8, pos ImVec2, cond ImGuiCond)

// set named window position.
@[c:'ImGui_SetWindowSizeStr']
fn im_gui_set_window_size_str(name &i8, size ImVec2, cond ImGuiCond)

// set named window size. set axis to 0.0f to force an auto-fit on this axis.
@[c:'ImGui_SetWindowCollapsedStr']
fn im_gui_set_window_collapsed_str(name &i8, collapsed bool, cond ImGuiCond)

// set named window collapsed state
@[c:'ImGui_SetWindowFocusStr']
fn im_gui_set_window_focus_str(name &i8)
...
@[c:'ImGui_GetScrollX']
fn im_gui_get_scroll_x() f32

// get scrolling amount [0 .. GetScrollMaxX()]
@[c:'ImGui_GetScrollY']
fn im_gui_get_scroll_y() f32

// get scrolling amount [0 .. GetScrollMaxY()]
@[c:'ImGui_SetScrollX']
fn im_gui_set_scroll_x(scroll_x f32)

// set scrolling amount [0 .. GetScrollMaxX()]
@[c:'ImGui_SetScrollY']
fn im_gui_set_scroll_y(scroll_y f32)

// set scrolling amount [0 .. GetScrollMaxY()]
@[c:'ImGui_GetScrollMaxX']
fn im_gui_get_scroll_max_x() f32

// get maximum scrolling amount ~~ ContentSize.x - WindowSize.x - DecorationsSize.x
@[c:'ImGui_GetScrollMaxY']
fn im_gui_get_scroll_max_y() f32

// get maximum scrolling amount ~~ ContentSize.y - WindowSize.y - DecorationsSize.y
@[c:'ImGui_SetScrollHereX']
fn im_gui_set_scroll_here_x(center_x_ratio f32)

// adjust scrolling amount to make current cursor position visible. center_x_ratio=0.0: left, 0.5: center, 1.0: right. When using to make a "default/current item" visible, consider using SetItemDefaultFocus() instead.
@[c:'ImGui_SetScrollHereY']
fn im_gui_set_scroll_here_y(center_y_ratio f32)

// adjust scrolling amount to make current cursor position visible. center_y_ratio=0.0: top, 0.5: center, 1.0: bottom. When using to make a "default/current item" visible, consider using SetItemDefaultFocus() instead.
@[c:'ImGui_SetScrollFromPosX']
fn im_gui_set_scroll_from_pos_x(local_x f32, center_x_ratio f32)

// adjust scrolling amount to make given position visible. Generally GetCursorStartPos() + offset to compute a valid position.
@[c:'ImGui_SetScrollFromPosY']
fn im_gui_set_scroll_from_pos_y(local_y f32, center_y_ratio f32)

// adjust scrolling amount to make given position visible. Generally GetCursorStartPos() + offset to compute a valid position.
// Parameters stacks (shared)
@[c:'ImGui_PushFont']
fn im_gui_push_font(font &ImFont)
...
fn im_gui_push_style_color(idx ImGuiCol, col ImU32)

// modify a style color. always use this if you modify the style after NewFrame().
@[c:'ImGui_PushStyleColorImVec4']
fn im_gui_push_style_color_im_vec4(idx ImGuiCol, col ImVec4)

@[c:'ImGui_PopStyleColor']
fn im_gui_pop_style_color()

// Implied count = 1
@[c:'ImGui_PopStyleColorEx']
fn im_gui_pop_style_color_ex(count int)

@[c:'ImGui_PushStyleVar']
fn im_gui_push_style_var(idx ImGuiStyleVar, val f32)

// modify a style float variable. always use this if you modify the style after NewFrame()!
@[c:'ImGui_PushStyleVarImVec2']
fn im_gui_push_style_var_im_vec2(idx ImGuiStyleVar, val ImVec2)

// modify a style ImVec2 variable. "
@[c:'ImGui_PushStyleVarX']
fn im_gui_push_style_var_x(idx ImGuiStyleVar, val_x f32)

// modify X component of a style ImVec2 variable. "
@[c:'ImGui_PushStyleVarY']
fn im_gui_push_style_var_y(idx ImGuiStyleVar, val_y f32)

// modify Y component of a style ImVec2 variable. "
@[c:'ImGui_PopStyleVar']
fn im_gui_pop_style_var()

// Implied count = 1
@[c:'ImGui_PopStyleVarEx']
fn im_gui_pop_style_var_ex(count int)

@[c:'ImGui_PushItemFlag']
fn im_gui_push_item_flag(option ImGuiItemFlags, enabled bool)

// modify specified shared item flag, e.g. PushItemFlag(ImGuiItemFlags_NoTabStop, true)
@[c:'ImGui_PopItemFlag']
fn im_gui_pop_item_flag()

// Parameters stacks (current window)
@[c:'ImGui_PushItemWidth']
fn im_gui_push_item_width(item_width f32)

// push width of items for common large "item+label" widgets. >0.0f: width in pixels, <0.0f align xx pixels to the right of window (so -FLT_MIN always align width to the right side).
@[c:'ImGui_PopItemWidth']
fn im_gui_pop_item_width()

@[c:'ImGui_SetNextItemWidth']
fn im_gui_set_next_item_width(item_width f32)

// set width of the _next_ common large "item+label" widget. >0.0f: width in pixels, <0.0f align xx pixels to the right of window (so -FLT_MIN always align width to the right side)
@[c:'ImGui_CalcItemWidth']
fn im_gui_calc_item_width() f32

// width of item given pushed settings and current cursor position. NOT necessarily the width of last item unlike most 'Item' functions.
@[c:'ImGui_PushTextWrapPos']
fn im_gui_push_text_wrap_pos(wrap_local_pos_x f32)

// push word-wrapping position for Text*() commands. < 0.0f: no wrapping; 0.0f: wrap to end of window (or column); > 0.0f: wrap at 'wrap_pos_x' position in window local space
@[c:'ImGui_PopTextWrapPos']
fn im_gui_pop_text_wrap_pos()

// Style read access
// - Use the ShowStyleEditor() function to interactively see/edit the colors.
@[c:'ImGui_GetFont']
fn im_gui_get_font() &ImFont

// get current font
@[c:'ImGui_GetFontSize']
fn im_gui_get_font_size() f32

// get current font size (= height in pixels) of current font with current scale applied
@[c:'ImGui_GetFontTexUvWhitePixel']
fn im_gui_get_font_tex_uv_white_pixel() ImVec2

// get UV coordinate for a white pixel, useful to draw custom shapes via the ImDrawList API
@[c:'ImGui_GetColorU32']
fn im_gui_get_color_u32(idx ImGuiCol) ImU32

// Implied alpha_mul = 1.0f
@[c:'ImGui_GetColorU32Ex']
fn im_gui_get_color_u32_ex(idx ImGuiCol, alpha_mul f32) ImU32

// retrieve given style color with style alpha applied and optional extra alpha multiplier, packed as a 32-bit value suitable for ImDrawList
@[c:'ImGui_GetColorU32ImVec4']
fn im_gui_get_color_u32_im_vec4(col ImVec4) ImU32

// retrieve given color with style alpha applied, packed as a 32-bit value suitable for ImDrawList
@[c:'ImGui_GetColorU32ImU32']
fn im_gui_get_color_u32_im_u32(col ImU32) ImU32

// Implied alpha_mul = 1.0f
@[c:'ImGui_GetColorU32ImU32Ex']
fn im_gui_get_color_u32_im_u32_ex(col ImU32, alpha_mul f32) ImU32

// retrieve given color with style alpha applied, packed as a 32-bit value suitable for ImDrawList
@[c:'ImGui_GetStyleColorVec4']
fn im_gui_get_style_color_vec4(idx ImGuiCol) &ImVec4
...
fn im_gui_get_cursor_screen_pos() ImVec2

// cursor position, absolute coordinates. THIS IS YOUR BEST FRIEND (prefer using this rather than GetCursorPos(), also more useful to work with ImDrawList API).
@[c:'ImGui_SetCursorScreenPos']
fn im_gui_set_cursor_screen_pos(pos ImVec2)

// cursor position, absolute coordinates. THIS IS YOUR BEST FRIEND.
@[c:'ImGui_GetContentRegionAvail']
fn im_gui_get_content_region_avail() ImVec2

// available space from current position. THIS IS YOUR BEST FRIEND.
@[c:'ImGui_GetCursorPos']
fn im_gui_get_cursor_pos() ImVec2

// [window-local] cursor position in window-local coordinates. This is not your best friend.
@[c:'ImGui_GetCursorPosX']
fn im_gui_get_cursor_pos_x() f32

// [window-local] "
@[c:'ImGui_GetCursorPosY']
fn im_gui_get_cursor_pos_y() f32

// [window-local] "
@[c:'ImGui_SetCursorPos']
fn im_gui_set_cursor_pos(local_pos ImVec2)

// [window-local] "
@[c:'ImGui_SetCursorPosX']
fn im_gui_set_cursor_pos_x(local_x f32)

// [window-local] "
@[c:'ImGui_SetCursorPosY']
fn im_gui_set_cursor_pos_y(local_y f32)

// [window-local] "
@[c:'ImGui_GetCursorStartPos']
fn im_gui_get_cursor_start_pos() ImVec2
...
@[c:'ImGui_SameLineEx']
fn im_gui_same_line_ex(offset_from_start_x f32, spacing f32)
...
@[c:'ImGui_IndentEx']
fn im_gui_indent_ex(indent_w f32)
...
// Implied indent_w = 0.0f
@[c:'ImGui_UnindentEx']
fn im_gui_unindent_ex(indent_w f32)

// move content position back to the left, by indent_w, or style.IndentSpacing if indent_w <= 0
@[c:'ImGui_BeginGroup']
fn im_gui_begin_group()
...
fn im_gui_align_text_to_frame_padding()

// vertically align upcoming text baseline to FramePadding.y so that it will align properly to regularly framed items (call if you have text on a line before a framed item)
@[c:'ImGui_GetTextLineHeight']
fn im_gui_get_text_line_height() f32

// ~ FontSize
@[c:'ImGui_GetTextLineHeightWithSpacing']
fn im_gui_get_text_line_height_with_spacing() f32

// ~ FontSize + style.ItemSpacing.y (distance in pixels between 2 consecutive lines of text)
@[c:'ImGui_GetFrameHeight']
fn im_gui_get_frame_height() f32

// ~ FontSize + style.FramePadding.y * 2
@[c:'ImGui_GetFrameHeightWithSpacing']
fn im_gui_get_frame_height_with_spacing() f32
...
@[c:'ImGui_PushID']
fn im_gui_push_id(str_id &i8)

// push string into the ID stack (will hash string).
@[c:'ImGui_PushIDStr']
fn im_gui_push_ids_tr(str_id_begin &i8, str_id_end &i8)

// push string into the ID stack (will hash string).
@[c:'ImGui_PushIDPtr']
fn im_gui_push_idp_tr(ptr_id voidptr)

// push pointer into the ID stack (will hash pointer).
@[c:'ImGui_PushIDInt']
fn im_gui_push_idi_nt(int_id int)
...
@[c:'ImGui_GetID']
fn im_gui_get_id(str_id &i8) ImGuiID

// calculate unique ID (hash of whole ID stack + given parameter). e.g. if you want to query into ImGuiStorage yourself
@[c:'ImGui_GetIDStr']
fn im_gui_get_ids_tr(str_id_begin &i8, str_id_end &i8) ImGuiID

@[c:'ImGui_GetIDPtr']
fn im_gui_get_idp_tr(ptr_id voidptr) ImGuiID

@[c:'ImGui_GetIDInt']
fn im_gui_get_idi_nt(int_id int) ImGuiID

// Widgets: Text
@[c:'ImGui_TextUnformatted']
fn im_gui_text_unformatted(text &i8)

// Implied text_end = NULL
@[c:'ImGui_TextUnformattedEx']
fn im_gui_text_unformatted_ex(text &i8, text_end &i8)

// raw text without formatting. Roughly equivalent to Text("%s", text) but: A) doesn't require null terminated string if 'text_end' is specified, B) it's faster, no memory copy is done, no buffer size limits, recommended for long chunks of text.
@[c2v_variadic]
@[c:'ImGui_Text']
fn im_gui_text(fmt &i8)

// formatted text
@[c:'ImGui_TextV']
fn im_gui_text_v(fmt &i8, args va_list)

@[c2v_variadic]
@[c:'ImGui_TextColored']
fn im_gui_text_colored(col ImVec4, fmt &i8)

// shortcut for PushStyleColor(ImGuiCol_Text, col); Text(fmt, ...); PopStyleColor();
@[c:'ImGui_TextColoredV']
fn im_gui_text_colored_v(col ImVec4, fmt &i8, args va_list)

@[c2v_variadic]
@[c:'ImGui_TextDisabled']
fn im_gui_text_disabled(fmt &i8)

// shortcut for PushStyleColor(ImGuiCol_Text, style.Colors[ImGuiCol_TextDisabled]); Text(fmt, ...); PopStyleColor();
@[c:'ImGui_TextDisabledV']
fn im_gui_text_disabled_v(fmt &i8, args va_list)

@[c2v_variadic]
@[c:'ImGui_TextWrapped']
fn im_gui_text_wrapped(fmt &i8)

// shortcut for PushTextWrapPos(0.0f); Text(fmt, ...); PopTextWrapPos();. Note that this won't work on an auto-resizing window if there's no other widgets to extend the window width, yoy may need to set a size using SetNextWindowSize().
@[c:'ImGui_TextWrappedV']
fn im_gui_text_wrapped_v(fmt &i8, args va_list)

@[c2v_variadic]
@[c:'ImGui_LabelText']
fn im_gui_label_text(label &i8, fmt &i8)

// display text+label aligned the same way as value+label widgets
@[c:'ImGui_LabelTextV']
fn im_gui_label_text_v(label &i8, fmt &i8, args va_list)

@[c2v_variadic]
@[c:'ImGui_BulletText']
fn im_gui_bullet_text(fmt &i8)

// shortcut for Bullet()+Text()
@[c:'ImGui_BulletTextV']
fn im_gui_bullet_text_v(fmt &i8, args va_list)

@[c:'ImGui_SeparatorText']
fn im_gui_separator_text(label &i8)
...
@[c:'ImGui_Button']
fn im_gui_button(label &i8) bool

// Implied size = ImVec2(0, 0)
@[c:'ImGui_ButtonEx']
fn im_gui_button_ex(label &i8, size ImVec2) bool

// button
@[c:'ImGui_SmallButton']
fn im_gui_small_button(label &i8) bool

// button with (FramePadding.y == 0) to easily embed within text
@[c:'ImGui_InvisibleButton']
fn im_gui_invisible_button(str_id &i8, size ImVec2, flags ImGuiButtonFlags) bool

// flexible button behavior without the visuals, frequently useful to build custom behaviors using the public api (along with IsItemActive, IsItemHovered, etc.)
@[c:'ImGui_ArrowButton']
fn im_gui_arrow_button(str_id &i8, dir ImGuiDir) bool

// square button with an arrow shape
@[c:'ImGui_Checkbox']
fn im_gui_checkbox(label &i8, v &bool) bool

@[c:'ImGui_CheckboxFlagsIntPtr']
fn im_gui_checkbox_flags_int_ptr(label &i8, flags &int, flags_value int) bool

@[c:'ImGui_CheckboxFlagsUintPtr']
fn im_gui_checkbox_flags_uint_ptr(label &i8, flags &u32, flags_value u32) bool

@[c:'ImGui_RadioButton']
fn im_gui_radio_button(label &i8, active bool) bool

// use with e.g. if (RadioButton("one", my_value==1)) { my_value = 1; }
@[c:'ImGui_RadioButtonIntPtr']
fn im_gui_radio_button_int_ptr(label &i8, v &int, v_button int) bool

// shortcut to handle the above pattern when value is an integer
@[c:'ImGui_ProgressBar']
fn im_gui_progress_bar(fraction f32, size_arg ImVec2, overlay &i8)
...
@[c:'ImGui_TextLink']
fn im_gui_text_link(label &i8) bool

// hyperlink text button, return true when clicked
@[c:'ImGui_TextLinkOpenURL']
fn im_gui_text_link_open_url(label &i8)

// Implied url = NULL
@[c:'ImGui_TextLinkOpenURLEx']
fn im_gui_text_link_open_urle_x(label &i8, url &i8)
...
@[c:'ImGui_Image']
fn im_gui_image(user_texture_id ImTextureID, image_size ImVec2)

// Implied uv0 = ImVec2(0, 0), uv1 = ImVec2(1, 1)
@[c:'ImGui_ImageEx']
fn im_gui_image_ex(user_texture_id ImTextureID, image_size ImVec2, uv0 ImVec2, uv1 ImVec2)

@[c:'ImGui_ImageWithBg']
fn im_gui_image_with_bg(user_texture_id ImTextureID, image_size ImVec2)

// Implied uv0 = ImVec2(0, 0), uv1 = ImVec2(1, 1), bg_col = ImVec4(0, 0, 0, 0), tint_col = ImVec4(1, 1, 1, 1)
@[c:'ImGui_ImageWithBgEx']
fn im_gui_image_with_bg_ex(user_texture_id ImTextureID, image_size ImVec2, uv0 ImVec2, uv1 ImVec2, bg_col ImVec4, tint_col ImVec4)

@[c:'ImGui_ImageButton']
fn im_gui_image_button(str_id &i8, user_texture_id ImTextureID, image_size ImVec2) bool

// Implied uv0 = ImVec2(0, 0), uv1 = ImVec2(1, 1), bg_col = ImVec4(0, 0, 0, 0), tint_col = ImVec4(1, 1, 1, 1)
@[c:'ImGui_ImageButtonEx']
fn im_gui_image_button_ex(str_id &i8, user_texture_id ImTextureID, image_size ImVec2, uv0 ImVec2, uv1 ImVec2, bg_col ImVec4, tint_col ImVec4) bool

// Widgets: Combo Box (Dropdown)
// - The BeginCombo()/EndCombo() api allows you to manage your contents and selection state however you want it, by creating e.g. Selectable() items.
// - The old Combo() api are helpers over BeginCombo()/EndCombo() which are kept available for convenience purpose. This is analogous to how ListBox are created.
@[c:'ImGui_BeginCombo']
fn im_gui_begin_combo(label &i8, preview_value &i8, flags ImGuiComboFlags) bool
...
@[c:'ImGui_ComboChar']
fn im_gui_combo_char(label &i8, current_item &int, items &&u8, items_count int) bool

// Implied popup_max_height_in_items = -1
@[c:'ImGui_ComboCharEx']
fn im_gui_combo_char_ex(label &i8, current_item &int, items &&u8, items_count int, popup_max_height_in_items int) bool

@[c:'ImGui_Combo']
fn im_gui_combo(label &i8, current_item &int, items_separated_by_zeros &i8) bool

// Implied popup_max_height_in_items = -1
@[c:'ImGui_ComboEx']
fn im_gui_combo_ex(label &i8, current_item &int, items_separated_by_zeros &i8, popup_max_height_in_items int) bool

// Separate items with \0 within a string, end item-list with \0\0. e.g. "One\0Two\0Three\0"
@[c:'ImGui_ComboCallback']
fn im_gui_combo_callback(label &i8, current_item &int, getter fn (voidptr, int) &i8, user_data voidptr, items_count int) bool

// Implied popup_max_height_in_items = -1
@[c:'ImGui_ComboCallbackEx']
fn im_gui_combo_callback_ex(label &i8, current_item &int, getter fn (voidptr, int) &i8, user_data voidptr, items_count int, popup_max_height_in_items int) bool
...
@[c:'ImGui_DragFloat']
fn im_gui_drag_float(label &i8, v &f32) bool

// Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", flags = 0
@[c:'ImGui_DragFloatEx']
fn im_gui_drag_float_ex(label &i8, v &f32, v_speed f32, v_min f32, v_max f32, format &i8, flags ImGuiSliderFlags) bool

// If v_min >= v_max we have no bound
@[c:'ImGui_DragFloat2']
fn im_gui_drag_float2(label &i8, v &f32) bool

// Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", flags = 0
@[c:'ImGui_DragFloat2Ex']
fn im_gui_drag_float2_ex(label &i8, v &f32, v_speed f32, v_min f32, v_max f32, format &i8, flags ImGuiSliderFlags) bool

@[c:'ImGui_DragFloat3']
fn im_gui_drag_float3(label &i8, v &f32) bool

// Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", flags = 0
@[c:'ImGui_DragFloat3Ex']
fn im_gui_drag_float3_ex(label &i8, v &f32, v_speed f32, v_min f32, v_max f32, format &i8, flags ImGuiSliderFlags) bool

@[c:'ImGui_DragFloat4']
fn im_gui_drag_float4(label &i8, v &f32) bool

// Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", flags = 0
@[c:'ImGui_DragFloat4Ex']
fn im_gui_drag_float4_ex(label &i8, v &f32, v_speed f32, v_min f32, v_max f32, format &i8, flags ImGuiSliderFlags) bool

@[c:'ImGui_DragFloatRange2']
fn im_gui_drag_float_range2(label &i8, v_current_min &f32, v_current_max &f32) bool

// Implied v_speed = 1.0f, v_min = 0.0f, v_max = 0.0f, format = "%.3f", format_max = NULL, flags = 0
@[c:'ImGui_DragFloatRange2Ex']
fn im_gui_drag_float_range2_ex(label &i8, v_current_min &f32, v_current_max &f32, v_speed f32, v_min f32, v_max f32, format &i8, format_max &i8, flags ImGuiSliderFlags) bool

@[c:'ImGui_DragInt']
fn im_gui_drag_int(label &i8, v &int) bool

// Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", flags = 0
@[c:'ImGui_DragIntEx']
fn im_gui_drag_int_ex(label &i8, v &int, v_speed f32, v_min int, v_max int, format &i8, flags ImGuiSliderFlags) bool

// If v_min >= v_max we have no bound
@[c:'ImGui_DragInt2']
fn im_gui_drag_int2(label &i8, v &int) bool

// Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", flags = 0
@[c:'ImGui_DragInt2Ex']
fn im_gui_drag_int2_ex(label &i8, v &int, v_speed f32, v_min int, v_max int, format &i8, flags ImGuiSliderFlags) bool

@[c:'ImGui_DragInt3']
fn im_gui_drag_int3(label &i8, v &int) bool

// Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", flags = 0
@[c:'ImGui_DragInt3Ex']
fn im_gui_drag_int3_ex(label &i8, v &int, v_speed f32, v_min int, v_max int, format &i8, flags ImGuiSliderFlags) bool

@[c:'ImGui_DragInt4']
fn im_gui_drag_int4(label &i8, v &int) bool

// Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", flags = 0
@[c:'ImGui_DragInt4Ex']
fn im_gui_drag_int4_ex(label &i8, v &int, v_speed f32, v_min int, v_max int, format &i8, flags ImGuiSliderFlags) bool

@[c:'ImGui_DragIntRange2']
fn im_gui_drag_int_range2(label &i8, v_current_min &int, v_current_max &int) bool

// Implied v_speed = 1.0f, v_min = 0, v_max = 0, format = "%d", format_max = NULL, flags = 0
@[c:'ImGui_DragIntRange2Ex']
fn im_gui_drag_int_range2_ex(label &i8, v_current_min &int, v_current_max &int, v_speed f32, v_min int, v_max int, format &i8, format_max &i8, flags ImGuiSliderFlags) bool

@[c:'ImGui_DragScalar']
fn im_gui_drag_scalar(label &i8, data_type ImGuiDataType, p_data voidptr) bool

// Implied v_speed = 1.0f, p_min = NULL, p_max = NULL, format = NULL, flags = 0
@[c:'ImGui_DragScalarEx']
fn im_gui_drag_scalar_ex(label &i8, data_type ImGuiDataType, p_data voidptr, v_speed f32, p_min voidptr, p_max voidptr, format &i8, flags ImGuiSliderFlags) bool

@[c:'ImGui_DragScalarN']
fn im_gui_drag_scalar_n(label &i8, data_type ImGuiDataType, p_data voidptr, components int) bool

// Implied v_speed = 1.0f, p_min = NULL, p_max = NULL, format = NULL, flags = 0
@[c:'ImGui_DragScalarNEx']
fn im_gui_drag_scalar_ne_x(label &i8, data_type ImGuiDataType, p_data voidptr, components int, v_speed f32, p_min voidptr, p_max voidptr, format &i8, flags ImGuiSliderFlags) bool
...
//   If you get a warning converting a float to ImGuiSliderFlags, read https://github.com/ocornut/imgui/issues/3361
@[c:'ImGui_SliderFloat']
fn im_gui_slider_float(label &i8, v &f32, v_min f32, v_max f32) bool

// Implied format = "%.3f", flags = 0
@[c:'ImGui_SliderFloatEx']
fn im_gui_slider_float_ex(label &i8, v &f32, v_min f32, v_max f32, format &i8, flags ImGuiSliderFlags) bool

// adjust format to decorate the value with a prefix or a suffix for in-slider labels or unit display.
@[c:'ImGui_SliderFloat2']
fn im_gui_slider_float2(label &i8, v &f32, v_min f32, v_max f32) bool

// Implied format = "%.3f", flags = 0
@[c:'ImGui_SliderFloat2Ex']
fn im_gui_slider_float2_ex(label &i8, v &f32, v_min f32, v_max f32, format &i8, flags ImGuiSliderFlags) bool

@[c:'ImGui_SliderFloat3']
fn im_gui_slider_float3(label &i8, v &f32, v_min f32, v_max f32) bool

// Implied format = "%.3f", flags = 0
@[c:'ImGui_SliderFloat3Ex']
fn im_gui_slider_float3_ex(label &i8, v &f32, v_min f32, v_max f32, format &i8, flags ImGuiSliderFlags) bool

@[c:'ImGui_SliderFloat4']
fn im_gui_slider_float4(label &i8, v &f32, v_min f32, v_max f32) bool

// Implied format = "%.3f", flags = 0
@[c:'ImGui_SliderFloat4Ex']
fn im_gui_slider_float4_ex(label &i8, v &f32, v_min f32, v_max f32, format &i8, flags ImGuiSliderFlags) bool

@[c:'ImGui_SliderAngle']
fn im_gui_slider_angle(label &i8, v_rad &f32) bool

// Implied v_degrees_min = -360.0f, v_degrees_max = +360.0f, format = "%.0f deg", flags = 0
@[c:'ImGui_SliderAngleEx']
fn im_gui_slider_angle_ex(label &i8, v_rad &f32, v_degrees_min f32, v_degrees_max f32, format &i8, flags ImGuiSliderFlags) bool

@[c:'ImGui_SliderInt']
fn im_gui_slider_int(label &i8, v &int, v_min int, v_max int) bool

// Implied format = "%d", flags = 0
@[c:'ImGui_SliderIntEx']
fn im_gui_slider_int_ex(label &i8, v &int, v_min int, v_max int, format &i8, flags ImGuiSliderFlags) bool

@[c:'ImGui_SliderInt2']
fn im_gui_slider_int2(label &i8, v &int, v_min int, v_max int) bool

// Implied format = "%d", flags = 0
@[c:'ImGui_SliderInt2Ex']
fn im_gui_slider_int2_ex(label &i8, v &int, v_min int, v_max int, format &i8, flags ImGuiSliderFlags) bool

@[c:'ImGui_SliderInt3']
fn im_gui_slider_int3(label &i8, v &int, v_min int, v_max int) bool

// Implied format = "%d", flags = 0
@[c:'ImGui_SliderInt3Ex']
fn im_gui_slider_int3_ex(label &i8, v &int, v_min int, v_max int, format &i8, flags ImGuiSliderFlags) bool

@[c:'ImGui_SliderInt4']
fn im_gui_slider_int4(label &i8, v &int, v_min int, v_max int) bool

// Implied format = "%d", flags = 0
@[c:'ImGui_SliderInt4Ex']
fn im_gui_slider_int4_ex(label &i8, v &int, v_min int, v_max int, format &i8, flags ImGuiSliderFlags) bool

@[c:'ImGui_SliderScalar']
fn im_gui_slider_scalar(label &i8, data_type ImGuiDataType, p_data voidptr, p_min voidptr, p_max voidptr) bool

// Implied format = NULL, flags = 0
@[c:'ImGui_SliderScalarEx']
fn im_gui_slider_scalar_ex(label &i8, data_type ImGuiDataType, p_data voidptr, p_min voidptr, p_max voidptr, format &i8, flags ImGuiSliderFlags) bool

@[c:'ImGui_SliderScalarN']
fn im_gui_slider_scalar_n(label &i8, data_type ImGuiDataType, p_data voidptr, components int, p_min voidptr, p_max voidptr) bool

// Implied format = NULL, flags = 0
@[c:'ImGui_SliderScalarNEx']
fn im_gui_slider_scalar_ne_x(label &i8, data_type ImGuiDataType, p_data voidptr, components int, p_min voidptr, p_max voidptr, format &i8, flags ImGuiSliderFlags) bool

@[c:'ImGui_VSliderFloat']
fn im_gui_vs_lider_float(label &i8, size ImVec2, v &f32, v_min f32, v_max f32) bool

// Implied format = "%.3f", flags = 0
@[c:'ImGui_VSliderFloatEx']
fn im_gui_vs_lider_float_ex(label &i8, size ImVec2, v &f32, v_min f32, v_max f32, format &i8, flags ImGuiSliderFlags) bool

@[c:'ImGui_VSliderInt']
fn im_gui_vs_lider_int(label &i8, size ImVec2, v &int, v_min int, v_max int) bool

// Implied format = "%d", flags = 0
@[c:'ImGui_VSliderIntEx']
fn im_gui_vs_lider_int_ex(label &i8, size ImVec2, v &int, v_min int, v_max int, format &i8, flags ImGuiSliderFlags) bool

@[c:'ImGui_VSliderScalar']
fn im_gui_vs_lider_scalar(label &i8, size ImVec2, data_type ImGuiDataType, p_data voidptr, p_min voidptr, p_max voidptr) bool

// Implied format = NULL, flags = 0
@[c:'ImGui_VSliderScalarEx']
fn im_gui_vs_lider_scalar_ex(label &i8, size ImVec2, data_type ImGuiDataType, p_data voidptr, p_min voidptr, p_max voidptr, format &i8, flags ImGuiSliderFlags) bool

// Widgets: Input with Keyboard
// - If you want to use InputText() with std::string or any custom dynamic string type, see misc/cpp/imgui_stdlib.h and comments in imgui_demo.cpp.
// - Most of the ImGuiInputTextFlags flags are only useful for InputText() and not for InputFloatX, InputIntX, InputDouble etc.
@[c:'ImGui_InputText']
fn im_gui_input_text(label &i8, buf &i8, buf_size usize, flags ImGuiInputTextFlags) bool

// Implied callback = NULL, user_data = NULL
@[c:'ImGui_InputTextEx']
fn im_gui_input_text_ex(label &i8, buf &i8, buf_size usize, flags ImGuiInputTextFlags, callback ImGuiInputTextCallback, user_data voidptr) bool

@[c:'ImGui_InputTextMultiline']
fn im_gui_input_text_multiline(label &i8, buf &i8, buf_size usize) bool

// Implied size = ImVec2(0, 0), flags = 0, callback = NULL, user_data = NULL
@[c:'ImGui_InputTextMultilineEx']
fn im_gui_input_text_multiline_ex(label &i8, buf &i8, buf_size usize, size ImVec2, flags ImGuiInputTextFlags, callback ImGuiInputTextCallback, user_data voidptr) bool

@[c:'ImGui_InputTextWithHint']
fn im_gui_input_text_with_hint(label &i8, hint &i8, buf &i8, buf_size usize, flags ImGuiInputTextFlags) bool

// Implied callback = NULL, user_data = NULL
@[c:'ImGui_InputTextWithHintEx']
fn im_gui_input_text_with_hint_ex(label &i8, hint &i8, buf &i8, buf_size usize, flags ImGuiInputTextFlags, callback ImGuiInputTextCallback, user_data voidptr) bool

@[c:'ImGui_InputFloat']
fn im_gui_input_float(label &i8, v &f32) bool

// Implied step = 0.0f, step_fast = 0.0f, format = "%.3f", flags = 0
@[c:'ImGui_InputFloatEx']
fn im_gui_input_float_ex(label &i8, v &f32, step f32, step_fast f32, format &i8, flags ImGuiInputTextFlags) bool

@[c:'ImGui_InputFloat2']
fn im_gui_input_float2(label &i8, v &f32) bool

// Implied format = "%.3f", flags = 0
@[c:'ImGui_InputFloat2Ex']
fn im_gui_input_float2_ex(label &i8, v &f32, format &i8, flags ImGuiInputTextFlags) bool

@[c:'ImGui_InputFloat3']
fn im_gui_input_float3(label &i8, v &f32) bool

// Implied format = "%.3f", flags = 0
@[c:'ImGui_InputFloat3Ex']
fn im_gui_input_float3_ex(label &i8, v &f32, format &i8, flags ImGuiInputTextFlags) bool

@[c:'ImGui_InputFloat4']
fn im_gui_input_float4(label &i8, v &f32) bool

// Implied format = "%.3f", flags = 0
@[c:'ImGui_InputFloat4Ex']
fn im_gui_input_float4_ex(label &i8, v &f32, format &i8, flags ImGuiInputTextFlags) bool

@[c:'ImGui_InputInt']
fn im_gui_input_int(label &i8, v &int) bool

// Implied step = 1, step_fast = 100, flags = 0
@[c:'ImGui_InputIntEx']
fn im_gui_input_int_ex(label &i8, v &int, step int, step_fast int, flags ImGuiInputTextFlags) bool

@[c:'ImGui_InputInt2']
fn im_gui_input_int2(label &i8, v &int, flags ImGuiInputTextFlags) bool

@[c:'ImGui_InputInt3']
fn im_gui_input_int3(label &i8, v &int, flags ImGuiInputTextFlags) bool

@[c:'ImGui_InputInt4']
fn im_gui_input_int4(label &i8, v &int, flags ImGuiInputTextFlags) bool

@[c:'ImGui_InputDouble']
fn im_gui_input_double(label &i8, v &f64) bool

// Implied step = 0.0, step_fast = 0.0, format = "%.6f", flags = 0
@[c:'ImGui_InputDoubleEx']
fn im_gui_input_double_ex(label &i8, v &f64, step f64, step_fast f64, format &i8, flags ImGuiInputTextFlags) bool

@[c:'ImGui_InputScalar']
fn im_gui_input_scalar(label &i8, data_type ImGuiDataType, p_data voidptr) bool

// Implied p_step = NULL, p_step_fast = NULL, format = NULL, flags = 0
@[c:'ImGui_InputScalarEx']
fn im_gui_input_scalar_ex(label &i8, data_type ImGuiDataType, p_data voidptr, p_step voidptr, p_step_fast voidptr, format &i8, flags ImGuiInputTextFlags) bool

@[c:'ImGui_InputScalarN']
fn im_gui_input_scalar_n(label &i8, data_type ImGuiDataType, p_data voidptr, components int) bool

// Implied p_step = NULL, p_step_fast = NULL, format = NULL, flags = 0
@[c:'ImGui_InputScalarNEx']
fn im_gui_input_scalar_ne_x(label &i8, data_type ImGuiDataType, p_data voidptr, components int, p_step voidptr, p_step_fast voidptr, format &i8, flags ImGuiInputTextFlags) bool

// Widgets: Color Editor/Picker (tip: the ColorEdit* functions have a little color square that can be left-clicked to open a picker, and right-clicked to open an option menu.)
// - Note that in C++ a 'float v[X]' function argument is the _same_ as 'float* v', the array syntax is just a way to document the number of elements that are expected to be accessible.
// - You can pass the address of a first float element out of a contiguous structure, e.g. &myvector.x
@[c:'ImGui_ColorEdit3']
fn im_gui_color_edit3(label &i8, col &f32, flags ImGuiColorEditFlags) bool

@[c:'ImGui_ColorEdit4']
fn im_gui_color_edit4(label &i8, col &f32, flags ImGuiColorEditFlags) bool

@[c:'ImGui_ColorPicker3']
fn im_gui_color_picker3(label &i8, col &f32, flags ImGuiColorEditFlags) bool

@[c:'ImGui_ColorPicker4']
fn im_gui_color_picker4(label &i8, col &f32, flags ImGuiColorEditFlags, ref_col &f32) bool

@[c:'ImGui_ColorButton']
fn im_gui_color_button(desc_id &i8, col ImVec4, flags ImGuiColorEditFlags) bool

// Implied size = ImVec2(0, 0)
@[c:'ImGui_ColorButtonEx']
fn im_gui_color_button_ex(desc_id &i8, col ImVec4, flags ImGuiColorEditFlags, size ImVec2) bool

// display a color square/button, hover for details, return true when pressed.
@[c:'ImGui_SetColorEditOptions']
fn im_gui_set_color_edit_options(flags ImGuiColorEditFlags)

// initialize current options (generally on application startup) if you want to select a default format, picker type, etc. User will be able to change many settings, unless you pass the _NoOptions flag to your calls.
// Widgets: Trees
// - TreeNode functions return true when the node is open, in which case you need to also call TreePop() when you are finished displaying the tree node contents.
@[c:'ImGui_TreeNode']
fn im_gui_tree_node(label &i8) bool

@[c2v_variadic]
@[c:'ImGui_TreeNodeStr']
fn im_gui_tree_node_str(str_id &i8, fmt &i8) bool

// helper variation to easily decorelate the id from the displayed string. Read the FAQ about why and how to use ID. to align arbitrary text at the same level as a TreeNode() you can use Bullet().
@[c2v_variadic]
@[c:'ImGui_TreeNodePtr']
fn im_gui_tree_node_ptr(ptr_id voidptr, fmt &i8) bool

// "
@[c:'ImGui_TreeNodeV']
fn im_gui_tree_node_v(str_id &i8, fmt &i8, args va_list) bool

@[c:'ImGui_TreeNodeVPtr']
fn im_gui_tree_node_vp_tr(ptr_id voidptr, fmt &i8, args va_list) bool

@[c:'ImGui_TreeNodeEx']
fn im_gui_tree_node_ex(label &i8, flags ImGuiTreeNodeFlags) bool

@[c2v_variadic]
@[c:'ImGui_TreeNodeExStr']
fn im_gui_tree_node_ex_str(str_id &i8, flags ImGuiTreeNodeFlags, fmt &i8) bool

@[c2v_variadic]
@[c:'ImGui_TreeNodeExPtr']
fn im_gui_tree_node_ex_ptr(ptr_id voidptr, flags ImGuiTreeNodeFlags, fmt &i8) bool

@[c:'ImGui_TreeNodeExV']
fn im_gui_tree_node_ex_v(str_id &i8, flags ImGuiTreeNodeFlags, fmt &i8, args va_list) bool

@[c:'ImGui_TreeNodeExVPtr']
fn im_gui_tree_node_ex_vp_tr(ptr_id voidptr, flags ImGuiTreeNodeFlags, fmt &i8, args va_list) bool

@[c:'ImGui_TreePush']
fn im_gui_tree_push(str_id &i8)

// ~ Indent()+PushID(). Already called by TreeNode() when returning true, but you can call TreePush/TreePop yourself if desired.
@[c:'ImGui_TreePushPtr']
fn im_gui_tree_push_ptr(ptr_id voidptr)
...
fn im_gui_get_tree_node_to_label_spacing() f32

// horizontal distance preceding label when using TreeNode*() or Bullet() == (g.FontSize + style.FramePadding.x*2) for a regular unframed TreeNode
@[c:'ImGui_CollapsingHeader']
fn im_gui_collapsing_header(label &i8, flags ImGuiTreeNodeFlags) bool

// if returning 'true' the header is open. doesn't indent nor push on ID stack. user doesn't have to call TreePop().
@[c:'ImGui_CollapsingHeaderBoolPtr']
fn im_gui_collapsing_header_bool_ptr(label &i8, p_visible &bool, flags ImGuiTreeNodeFlags) bool

// when 'p_visible != NULL': if '*p_visible==true' display an additional small close button on upper right of the header which will set the bool to false when clicked, if '*p_visible==false' don't display the header.
@[c:'ImGui_SetNextItemOpen']
fn im_gui_set_next_item_open(is_open bool, cond ImGuiCond)

// set next TreeNode/CollapsingHeader open state.
@[c:'ImGui_SetNextItemStorageID']
fn im_gui_set_next_item_storage_id(storage_id ImGuiID)
...
@[c:'ImGui_Selectable']
fn im_gui_selectable(label &i8) bool

// Implied selected = false, flags = 0, size = ImVec2(0, 0)
@[c:'ImGui_SelectableEx']
fn im_gui_selectable_ex(label &i8, selected bool, flags ImGuiSelectableFlags, size ImVec2) bool

// "bool selected" carry the selection state (read-only). Selectable() is clicked is returns true so you can modify your selection state. size.x==0.0: use remaining width, size.x>0.0: specify width. size.y==0.0: use label height, size.y>0.0: specify height
@[c:'ImGui_SelectableBoolPtr']
fn im_gui_selectable_bool_ptr(label &i8, p_selected &bool, flags ImGuiSelectableFlags) bool

// Implied size = ImVec2(0, 0)
@[c:'ImGui_SelectableBoolPtrEx']
fn im_gui_selectable_bool_ptr_ex(label &i8, p_selected &bool, flags ImGuiSelectableFlags, size ImVec2) bool
...
fn im_gui_begin_multi_select(flags ImGuiMultiSelectFlags) &ImGuiMultiSelectIO

// Implied selection_size = -1, items_count = -1
@[c:'ImGui_BeginMultiSelectEx']
fn im_gui_begin_multi_select_ex(flags ImGuiMultiSelectFlags, selection_size int, items_count int) &ImGuiMultiSelectIO

@[c:'ImGui_EndMultiSelect']
fn im_gui_end_multi_select() &ImGuiMultiSelectIO

@[c:'ImGui_SetNextItemSelectionUserData']
fn im_gui_set_next_item_selection_user_data(selection_user_data ImGuiSelectionUserData)

@[c:'ImGui_IsItemToggledSelection']
fn im_gui_is_item_toggled_selection() bool
...
fn im_gui_begin_list_box(label &i8, size ImVec2) bool

// open a framed scrolling region
@[c:'ImGui_EndListBox']
fn im_gui_end_list_box()

// only call EndListBox() if BeginListBox() returned true!
@[c:'ImGui_ListBox']
fn im_gui_list_box(label &i8, current_item &int, items &&u8, items_count int, height_in_items int) bool

@[c:'ImGui_ListBoxCallback']
fn im_gui_list_box_callback(label &i8, current_item &int, getter fn (voidptr, int) &i8, user_data voidptr, items_count int) bool

// Implied height_in_items = -1
@[c:'ImGui_ListBoxCallbackEx']
fn im_gui_list_box_callback_ex(label &i8, current_item &int, getter fn (voidptr, int) &i8, user_data voidptr, items_count int, height_in_items int) bool

// Widgets: Data Plotting
// - Consider using ImPlot (https://github.com/epezent/implot) which is much better!
@[c:'ImGui_PlotLines']
fn im_gui_plot_lines(label &i8, values &f32, values_count int)

// Implied values_offset = 0, overlay_text = NULL, scale_min = FLT_MAX, scale_max = FLT_MAX, graph_size = ImVec2(0, 0), stride = sizeof(float)
@[c:'ImGui_PlotLinesEx']
fn im_gui_plot_lines_ex(label &i8, values &f32, values_count int, values_offset int, overlay_text &i8, scale_min f32, scale_max f32, graph_size ImVec2, stride int)

@[c:'ImGui_PlotLinesCallback']
fn im_gui_plot_lines_callback(label &i8, values_getter fn (voidptr, int) f32, data voidptr, values_count int)
...
fn im_gui_plot_lines_callback_ex(label &i8, values_getter fn (voidptr, int) f32, data voidptr, values_count int, values_offset int, overlay_text &i8, scale_min f32, scale_max f32, graph_size ImVec2)

@[c:'ImGui_PlotHistogram']
fn im_gui_plot_histogram(label &i8, values &f32, values_count int)

// Implied values_offset = 0, overlay_text = NULL, scale_min = FLT_MAX, scale_max = FLT_MAX, graph_size = ImVec2(0, 0), stride = sizeof(float)
@[c:'ImGui_PlotHistogramEx']
fn im_gui_plot_histogram_ex(label &i8, values &f32, values_count int, values_offset int, overlay_text &i8, scale_min f32, scale_max f32, graph_size ImVec2, stride int)

@[c:'ImGui_PlotHistogramCallback']
fn im_gui_plot_histogram_callback(label &i8, values_getter fn (voidptr, int) f32, data voidptr, values_count int)

// Implied values_offset = 0, overlay_text = NULL, scale_min = FLT_MAX, scale_max = FLT_MAX, graph_size = ImVec2(0, 0)
@[c:'ImGui_PlotHistogramCallbackEx']
fn im_gui_plot_histogram_callback_ex(label &i8, values_getter fn (voidptr, int) f32, data voidptr, values_count int, values_offset int, overlay_text &i8, scale_min f32, scale_max f32, graph_size ImVec2)
...
fn im_gui_begin_menu_bar() bool

// append to menu-bar of current window (requires ImGuiWindowFlags_MenuBar flag set on parent window).
@[c:'ImGui_EndMenuBar']
fn im_gui_end_menu_bar()

// only call EndMenuBar() if BeginMenuBar() returns true!
@[c:'ImGui_BeginMainMenuBar']
fn im_gui_begin_main_menu_bar() bool

// create and append to a full screen menu-bar.
@[c:'ImGui_EndMainMenuBar']
fn im_gui_end_main_menu_bar()

// only call EndMainMenuBar() if BeginMainMenuBar() returns true!
@[c:'ImGui_BeginMenu']
fn im_gui_begin_menu(label &i8) bool

// Implied enabled = true
@[c:'ImGui_BeginMenuEx']
fn im_gui_begin_menu_ex(label &i8, enabled bool) bool
...
@[c:'ImGui_MenuItem']
fn im_gui_menu_item(label &i8) bool

// Implied shortcut = NULL, selected = false, enabled = true
@[c:'ImGui_MenuItemEx']
fn im_gui_menu_item_ex(label &i8, shortcut &i8, selected bool, enabled bool) bool

// return true when activated.
@[c:'ImGui_MenuItemBoolPtr']
fn im_gui_menu_item_bool_ptr(label &i8, shortcut &i8, p_selected &bool, enabled bool) bool
...
fn im_gui_begin_tooltip() bool

// begin/append a tooltip window.
@[c:'ImGui_EndTooltip']
fn im_gui_end_tooltip()

// only call EndTooltip() if BeginTooltip()/BeginItemTooltip() returns true!
@[c2v_variadic]
@[c:'ImGui_SetTooltip']
fn im_gui_set_tooltip(fmt &i8)

// set a text-only tooltip. Often used after a ImGui::IsItemHovered() check. Override any previous call to SetTooltip().
@[c:'ImGui_SetTooltipV']
fn im_gui_set_tooltip_v(fmt &i8, args va_list)
...
fn im_gui_begin_item_tooltip() bool

// begin/append a tooltip window if preceding item was hovered.
@[c2v_variadic]
@[c:'ImGui_SetItemTooltip']
fn im_gui_set_item_tooltip(fmt &i8)

// set a text-only tooltip if preceding item was hovered. override any previous call to SetTooltip().
@[c:'ImGui_SetItemTooltipV']
fn im_gui_set_item_tooltip_v(fmt &i8, args va_list)
...
@[c:'ImGui_BeginPopup']
fn im_gui_begin_popup(str_id &i8, flags ImGuiWindowFlags) bool

// return true if the popup is open, and you can start outputting to it.
@[c:'ImGui_BeginPopupModal']
fn im_gui_begin_popup_modal(name &i8, p_open &bool, flags ImGuiWindowFlags) bool
...
@[c:'ImGui_OpenPopup']
fn im_gui_open_popup(str_id &i8, popup_flags ImGuiPopupFlags)

// call to mark popup as open (don't call every frame!).
@[c:'ImGui_OpenPopupID']
fn im_gui_open_popup_id(id ImGuiID, popup_flags ImGuiPopupFlags)

// id overload to facilitate calling from nested stacks
@[c:'ImGui_OpenPopupOnItemClick']
fn im_gui_open_popup_on_item_click(str_id &i8, popup_flags ImGuiPopupFlags)

// helper to open popup when clicked on last item. Default to ImGuiPopupFlags_MouseButtonRight == 1. (note: actually triggers on the mouse _released_ event to be consistent with popup behaviors)
@[c:'ImGui_CloseCurrentPopup']
fn im_gui_close_current_popup()
...
fn im_gui_begin_popup_context_item() bool

// Implied str_id = NULL, popup_flags = 1
@[c:'ImGui_BeginPopupContextItemEx']
fn im_gui_begin_popup_context_item_ex(str_id &i8, popup_flags ImGuiPopupFlags) bool

// open+begin popup when clicked on last item. Use str_id==NULL to associate the popup to previous item. If you want to use that on a non-interactive item such as Text() you need to pass in an explicit ID here. read comments in .cpp!
@[c:'ImGui_BeginPopupContextWindow']
fn im_gui_begin_popup_context_window() bool

// Implied str_id = NULL, popup_flags = 1
@[c:'ImGui_BeginPopupContextWindowEx']
fn im_gui_begin_popup_context_window_ex(str_id &i8, popup_flags ImGuiPopupFlags) bool

// open+begin popup when clicked on current window.
@[c:'ImGui_BeginPopupContextVoid']
fn im_gui_begin_popup_context_void() bool

// Implied str_id = NULL, popup_flags = 1
@[c:'ImGui_BeginPopupContextVoidEx']
fn im_gui_begin_popup_context_void_ex(str_id &i8, popup_flags ImGuiPopupFlags) bool
...
@[c:'ImGui_IsPopupOpen']
fn im_gui_is_popup_open(str_id &i8, flags ImGuiPopupFlags) bool
...
// - 5. Call EndTable()
@[c:'ImGui_BeginTable']
fn im_gui_begin_table(str_id &i8, columns int, flags ImGuiTableFlags) bool

// Implied outer_size = ImVec2(0.0f, 0.0f), inner_width = 0.0f
@[c:'ImGui_BeginTableEx']
fn im_gui_begin_table_ex(str_id &i8, columns int, flags ImGuiTableFlags, outer_size ImVec2, inner_width f32) bool
...
fn im_gui_table_next_row()

// Implied row_flags = 0, min_row_height = 0.0f
@[c:'ImGui_TableNextRowEx']
fn im_gui_table_next_row_ex(row_flags ImGuiTableRowFlags, min_row_height f32)

// append into the first cell of a new row.
@[c:'ImGui_TableNextColumn']
fn im_gui_table_next_column() bool

// append into the next column (or first column of next row if currently in last column). Return true when column is visible.
@[c:'ImGui_TableSetColumnIndex']
fn im_gui_table_set_column_index(column_n int) bool
...
fn im_gui_table_setup_column(label &i8, flags ImGuiTableColumnFlags)

// Implied init_width_or_weight = 0.0f, user_id = 0
@[c:'ImGui_TableSetupColumnEx']
fn im_gui_table_setup_column_ex(label &i8, flags ImGuiTableColumnFlags, init_width_or_weight f32, user_id ImGuiID)

@[c:'ImGui_TableSetupScrollFreeze']
fn im_gui_table_setup_scroll_freeze(cols int, rows int)

// lock columns/rows so they stay visible when scrolled.
@[c:'ImGui_TableHeader']
fn im_gui_table_header(label &i8)

// submit one header cell manually (rarely used)
@[c:'ImGui_TableHeadersRow']
fn im_gui_table_headers_row()

// submit a row with headers cells based on data provided to TableSetupColumn() + submit context menu
@[c:'ImGui_TableAngledHeadersRow']
fn im_gui_table_angled_headers_row()
...
fn im_gui_table_get_sort_specs() &ImGuiTableSortSpecs

// get latest sort specs for the table (NULL if not sorting).  Lifetime: don't hold on this pointer over multiple frames or past any subsequent call to BeginTable().
@[c:'ImGui_TableGetColumnCount']
fn im_gui_table_get_column_count() int

// return number of columns (value passed to BeginTable)
@[c:'ImGui_TableGetColumnIndex']
fn im_gui_table_get_column_index() int

// return current column index.
@[c:'ImGui_TableGetRowIndex']
fn im_gui_table_get_row_index() int

// return current row index.
@[c:'ImGui_TableGetColumnName']
fn im_gui_table_get_column_name(column_n int) &i8

// return "" if column didn't have a name declared by TableSetupColumn(). Pass -1 to use current column.
@[c:'ImGui_TableGetColumnFlags']
fn im_gui_table_get_column_flags(column_n int) ImGuiTableColumnFlags

// return column flags so you can query their Enabled/Visible/Sorted/Hovered status flags. Pass -1 to use current column.
@[c:'ImGui_TableSetColumnEnabled']
fn im_gui_table_set_column_enabled(column_n int, v bool)

// change user accessible enabled/disabled state of a column. Set to false to hide the column. User can use the context menu to change this themselves (right-click in headers, or right-click in columns body with ImGuiTableFlags_ContextMenuInBody)
@[c:'ImGui_TableGetHoveredColumn']
fn im_gui_table_get_hovered_column() int

// return hovered column. return -1 when table is not hovered. return columns_count if the unused space at the right of visible columns is hovered. Can also use (TableGetColumnFlags() & ImGuiTableColumnFlags_IsHovered) instead.
@[c:'ImGui_TableSetBgColor']
fn im_gui_table_set_bg_color(target ImGuiTableBgTarget, color ImU32, column_n int)
...
@[c:'ImGui_ColumnsEx']
fn im_gui_columns_ex(count int, id &i8, borders bool)

@[c:'ImGui_NextColumn']
fn im_gui_next_column()

// next column, defaults to current row or next row if the current row is finished
@[c:'ImGui_GetColumnIndex']
fn im_gui_get_column_index() int

// get current column index
@[c:'ImGui_GetColumnWidth']
fn im_gui_get_column_width(column_index int) f32

// get column width (in pixels). pass -1 to use current column
@[c:'ImGui_SetColumnWidth']
fn im_gui_set_column_width(column_index int, width f32)

// set column width (in pixels). pass -1 to use current column
@[c:'ImGui_GetColumnOffset']
fn im_gui_get_column_offset(column_index int) f32

// get position of column line (in pixels, from the left side of the contents region). pass -1 to use current column, otherwise 0..GetColumnsCount() inclusive. column 0 is typically 0.0f
@[c:'ImGui_SetColumnOffset']
fn im_gui_set_column_offset(column_index int, offset_x f32)

// set position of column line (in pixels, from the left side of the contents region). pass -1 to use current column
@[c:'ImGui_GetColumnsCount']
fn im_gui_get_columns_count() int

// Tab Bars, Tabs
// - Note: Tabs are automatically created by the docking system (when in 'docking' branch). Use this to create tab bars/tabs yourself.
@[c:'ImGui_BeginTabBar']
fn im_gui_begin_tab_bar(str_id &i8, flags ImGuiTabBarFlags) bool

// create and append into a TabBar
@[c:'ImGui_EndTabBar']
fn im_gui_end_tab_bar()

// only call EndTabBar() if BeginTabBar() returns true!
@[c:'ImGui_BeginTabItem']
fn im_gui_begin_tab_item(label &i8, p_open &bool, flags ImGuiTabItemFlags) bool

// create a Tab. Returns true if the Tab is selected.
@[c:'ImGui_EndTabItem']
fn im_gui_end_tab_item()

// only call EndTabItem() if BeginTabItem() returns true!
@[c:'ImGui_TabItemButton']
fn im_gui_tab_item_button(label &i8, flags ImGuiTabItemFlags) bool

// create a Tab behaving like a button. return true when clicked. cannot be selected in the tab bar.
@[c:'ImGui_SetTabItemClosed']
fn im_gui_set_tab_item_closed(tab_or_docked_window_label &i8)

// notify TabBar or Docking system of a closed tab/window ahead (useful to reduce visual flicker on reorderable tab bars). For tab-bar: call after BeginTabBar() and before Tab submissions. Otherwise call with a window name.
// Logging/Capture
// - All text output from the interface can be captured into tty/file/clipboard. By default, tree nodes are automatically opened during logging.
@[c:'ImGui_LogToTTY']
fn im_gui_log_to_tty(auto_open_depth int)

// start logging to tty (stdout)
@[c:'ImGui_LogToFile']
fn im_gui_log_to_file(auto_open_depth int, filename &i8)

// start logging to file
@[c:'ImGui_LogToClipboard']
fn im_gui_log_to_clipboard(auto_open_depth int)

// start logging to OS clipboard
@[c:'ImGui_LogFinish']
fn im_gui_log_finish()

// stop logging (close file, etc.)
@[c:'ImGui_LogButtons']
fn im_gui_log_buttons()

// helper to display buttons for logging to tty/file/clipboard
@[c2v_variadic]
@[c:'ImGui_LogText']
fn im_gui_log_text(fmt &i8)

// pass text data straight to log (without being displayed)
@[c:'ImGui_LogTextV']
fn im_gui_log_text_v(fmt &i8, args va_list)
...
fn im_gui_begin_drag_drop_source(flags ImGuiDragDropFlags) bool

// call after submitting an item which may be dragged. when this return true, you can call SetDragDropPayload() + EndDragDropSource()
@[c:'ImGui_SetDragDropPayload']
fn im_gui_set_drag_drop_payload(type_ &i8, data voidptr, sz usize, cond ImGuiCond) bool

// type is a user defined string of maximum 32 characters. Strings starting with '_' are reserved for dear imgui internal types. Data is copied and held by imgui. Return true when payload has been accepted.
@[c:'ImGui_EndDragDropSource']
fn im_gui_end_drag_drop_source()

// only call EndDragDropSource() if BeginDragDropSource() returns true!
@[c:'ImGui_BeginDragDropTarget']
fn im_gui_begin_drag_drop_target() bool

// call after submitting an item that may receive a payload. If this returns true, you can call AcceptDragDropPayload() + EndDragDropTarget()
@[c:'ImGui_AcceptDragDropPayload']
fn im_gui_accept_drag_drop_payload(type_ &i8, flags ImGuiDragDropFlags) &ImGuiPayload

// accept contents of a given type. If ImGuiDragDropFlags_AcceptBeforeDelivery is set you can peek into the payload before the mouse button is released.
@[c:'ImGui_EndDragDropTarget']
fn im_gui_end_drag_drop_target()

// only call EndDragDropTarget() if BeginDragDropTarget() returns true!
@[c:'ImGui_GetDragDropPayload']
fn im_gui_get_drag_drop_payload() &ImGuiPayload
...
fn im_gui_begin_disabled(disabled bool)

@[c:'ImGui_EndDisabled']
fn im_gui_end_disabled()

// Clipping
// - Mouse hovering is affected by ImGui::PushClipRect() calls, unlike direct calls to ImDrawList::PushClipRect() which are render only.
@[c:'ImGui_PushClipRect']
fn im_gui_push_clip_rect(clip_rect_min ImVec2, clip_rect_max ImVec2, intersect_with_current_clip_rect bool)

@[c:'ImGui_PopClipRect']
fn im_gui_pop_clip_rect()

// Focus, Activation
@[c:'ImGui_SetItemDefaultFocus']
fn im_gui_set_item_default_focus()

// make last item the default focused item of a newly appearing window.
@[c:'ImGui_SetKeyboardFocusHere']
fn im_gui_set_keyboard_focus_here()

// Implied offset = 0
@[c:'ImGui_SetKeyboardFocusHereEx']
fn im_gui_set_keyboard_focus_here_ex(offset int)

// focus keyboard on the next widget. Use positive 'offset' to access sub components of a multiple component widget. Use -1 to access previous widget.
// Keyboard/Gamepad Navigation
@[c:'ImGui_SetNavCursorVisible']
fn im_gui_set_nav_cursor_visible(visible bool)

// alter visibility of keyboard/gamepad cursor. by default: show when using an arrow key, hide when clicking with mouse.
// Overlapping mode
@[c:'ImGui_SetNextItemAllowOverlap']
fn im_gui_set_next_item_allow_overlap()
...
fn im_gui_get_main_viewport() &ImGuiViewport

// return primary/default viewport. This can never be NULL.
// Background/Foreground Draw Lists
@[c:'ImGui_GetBackgroundDrawList']
fn im_gui_get_background_draw_list() &ImDrawList
...
fn im_gui_get_foreground_draw_list() &ImDrawList

// this draw list will be the last rendered one. Useful to quickly draw shapes/text over dear imgui contents.
// Miscellaneous Utilities
@[c:'ImGui_IsRectVisibleBySize']
fn im_gui_is_rect_visible_by_size(size ImVec2) bool

// test if rectangle (of given size, starting from cursor position) is visible / not clipped.
@[c:'ImGui_IsRectVisible']
fn im_gui_is_rect_visible(rect_min ImVec2, rect_max ImVec2) bool

// test if rectangle (in screen space) is visible / not clipped. to perform coarse clipping on user's side.
@[c:'ImGui_GetTime']
fn im_gui_get_time() f64

// get global imgui time. incremented by io.DeltaTime every frame.
@[c:'ImGui_GetFrameCount']
fn im_gui_get_frame_count() int

// get global imgui frame count. incremented by 1 every frame.
@[c:'ImGui_GetDrawListSharedData']
fn im_gui_get_draw_list_shared_data() &ImDrawListSharedData

// you may use this when creating your own ImDrawList instances.
@[c:'ImGui_GetStyleColorName']
fn im_gui_get_style_color_name(idx ImGuiCol) &i8

// get a string corresponding to the enum value (for display, saving, etc.).
@[c:'ImGui_SetStateStorage']
fn im_gui_set_state_storage(storage &ImGuiStorage)

// replace current window storage with our own (if you want to manipulate it yourself, typically clear subsection of it)
@[c:'ImGui_GetStateStorage']
fn im_gui_get_state_storage() &ImGuiStorage

// Text Utilities
@[c:'ImGui_CalcTextSize']
fn im_gui_calc_text_size(text &i8) ImVec2

// Implied text_end = NULL, hide_text_after_double_hash = false, wrap_width = -1.0f
@[c:'ImGui_CalcTextSizeEx']
fn im_gui_calc_text_size_ex(text &i8, text_end &i8, hide_text_after_double_hash bool, wrap_width f32) ImVec2

// Color Utilities
@[c:'ImGui_ColorConvertU32ToFloat4']
fn im_gui_color_convert_u32_to_float4(in_ ImU32) ImVec4

@[c:'ImGui_ColorConvertFloat4ToU32']
fn im_gui_color_convert_float4_to_u32(in_ ImVec4) ImU32

@[c:'ImGui_ColorConvertRGBtoHSV']
fn im_gui_color_convert_rgb_to_hsv(r f32, g f32, b f32, out_h &f32, out_s &f32, out_v &f32)

@[c:'ImGui_ColorConvertHSVtoRGB']
fn im_gui_color_convert_hsv_to_rgb(h f32, s f32, v f32, out_r &f32, out_g &f32, out_b &f32)
...
@[c:'ImGui_IsKeyDown']
fn im_gui_is_key_down(key ImGuiKey) bool

// is key being held.
@[c:'ImGui_IsKeyPressed']
fn im_gui_is_key_pressed(key ImGuiKey) bool

// Implied repeat = true
@[c:'ImGui_IsKeyPressedEx']
fn im_gui_is_key_pressed_ex(key ImGuiKey, repeat bool) bool

// was key pressed (went from !Down to Down)? if repeat=true, uses io.KeyRepeatDelay / KeyRepeatRate
@[c:'ImGui_IsKeyReleased']
fn im_gui_is_key_released(key ImGuiKey) bool

// was key released (went from Down to !Down)?
@[c:'ImGui_IsKeyChordPressed']
fn im_gui_is_key_chord_pressed(key_chord ImGuiKeyChord) bool

// was key chord (mods + key) pressed, e.g. you can pass 'ImGuiMod_Ctrl | ImGuiKey_S' as a key-chord. This doesn't do any routing or focus check, please consider using Shortcut() function instead.
@[c:'ImGui_GetKeyPressedAmount']
fn im_gui_get_key_pressed_amount(key ImGuiKey, repeat_delay f32, rate f32) int

// uses provided repeat rate/delay. return a count, most often 0 or 1 but might be >1 if RepeatRate is small enough that DeltaTime > RepeatRate
@[c:'ImGui_GetKeyName']
fn im_gui_get_key_name(key ImGuiKey) &i8

// [DEBUG] returns English name of the key. Those names a provided for debugging purpose and are not meant to be saved persistently not compared.
@[c:'ImGui_SetNextFrameWantCaptureKeyboard']
fn im_gui_set_next_frame_want_capture_keyboard(want_capture_keyboard bool)
...
@[c:'ImGui_Shortcut']
fn im_gui_shortcut(key_chord ImGuiKeyChord, flags ImGuiInputFlags) bool

@[c:'ImGui_SetNextItemShortcut']
fn im_gui_set_next_item_shortcut(key_chord ImGuiKeyChord, flags ImGuiInputFlags)
...
fn im_gui_set_item_key_owner(key ImGuiKey)
...
@[c:'ImGui_IsMouseDown']
fn im_gui_is_mouse_down(button ImGuiMouseButton) bool

// is mouse button held?
@[c:'ImGui_IsMouseClicked']
fn im_gui_is_mouse_clicked(button ImGuiMouseButton) bool

// Implied repeat = false
@[c:'ImGui_IsMouseClickedEx']
fn im_gui_is_mouse_clicked_ex(button ImGuiMouseButton, repeat bool) bool

// did mouse button clicked? (went from !Down to Down). Same as GetMouseClickedCount() == 1.
@[c:'ImGui_IsMouseReleased']
fn im_gui_is_mouse_released(button ImGuiMouseButton) bool

// did mouse button released? (went from Down to !Down)
@[c:'ImGui_IsMouseDoubleClicked']
fn im_gui_is_mouse_double_clicked(button ImGuiMouseButton) bool

// did mouse button double-clicked? Same as GetMouseClickedCount() == 2. (note that a double-click will also report IsMouseClicked() == true)
@[c:'ImGui_IsMouseReleasedWithDelay']
fn im_gui_is_mouse_released_with_delay(button ImGuiMouseButton, delay f32) bool
...
fn im_gui_get_mouse_clicked_count(button ImGuiMouseButton) int

// return the number of successive mouse-clicks at the time where a click happen (otherwise 0).
@[c:'ImGui_IsMouseHoveringRect']
fn im_gui_is_mouse_hovering_rect(r_min ImVec2, r_max ImVec2) bool

// Implied clip = true
@[c:'ImGui_IsMouseHoveringRectEx']
fn im_gui_is_mouse_hovering_rect_ex(r_min ImVec2, r_max ImVec2, clip bool) bool

// is mouse hovering given bounding rect (in screen space). clipped by current clipping settings, but disregarding of other consideration of focus/window ordering/popup-block.
@[c:'ImGui_IsMousePosValid']
fn im_gui_is_mouse_pos_valid(mouse_pos &ImVec2) bool

// by convention we use (-FLT_MAX,-FLT_MAX) to denote that there is no mouse available
@[c:'ImGui_IsAnyMouseDown']
fn im_gui_is_any_mouse_down() bool

// [WILL OBSOLETE] is any mouse button held? This was designed for backends, but prefer having backend maintain a mask of held mouse buttons, because upcoming input queue system will make this invalid.
@[c:'ImGui_GetMousePos']
fn im_gui_get_mouse_pos() ImVec2

// shortcut to ImGui::GetIO().MousePos provided by user, to be consistent with other calls
@[c:'ImGui_GetMousePosOnOpeningCurrentPopup']
fn im_gui_get_mouse_pos_on_opening_current_popup() ImVec2

// retrieve mouse position at the time of opening popup we have BeginPopup() into (helper to avoid user backing that value themselves)
@[c:'ImGui_IsMouseDragging']
fn im_gui_is_mouse_dragging(button ImGuiMouseButton, lock_threshold f32) bool

// is mouse dragging? (uses io.MouseDraggingThreshold if lock_threshold < 0.0f)
@[c:'ImGui_GetMouseDragDelta']
fn im_gui_get_mouse_drag_delta(button ImGuiMouseButton, lock_threshold f32) ImVec2

// return the delta from the initial clicking position while the mouse button is pressed or was just released. This is locked and return 0.0f until the mouse moves past a distance threshold at least once (uses io.MouseDraggingThreshold if lock_threshold < 0.0f)
@[c:'ImGui_ResetMouseDragDelta']
fn im_gui_reset_mouse_drag_delta()

// Implied button = 0
@[c:'ImGui_ResetMouseDragDeltaEx']
fn im_gui_reset_mouse_drag_delta_ex(button ImGuiMouseButton)

//
@[c:'ImGui_GetMouseCursor']
fn im_gui_get_mouse_cursor() ImGuiMouseCursor

// get desired mouse cursor shape. Important: reset in ImGui::NewFrame(), this is updated during the frame. valid before Render(). If you use software rendering by setting io.MouseDrawCursor ImGui will render those for you
@[c:'ImGui_SetMouseCursor']
fn im_gui_set_mouse_cursor(cursor_type ImGuiMouseCursor)

// set desired mouse cursor shape
@[c:'ImGui_SetNextFrameWantCaptureMouse']
fn im_gui_set_next_frame_want_capture_mouse(want_capture_mouse bool)

// Override io.WantCaptureMouse flag next frame (said flag is left for your application to handle, typical when true it instucts your app to ignore inputs). This is equivalent to setting "io.WantCaptureMouse = want_capture_mouse;" after the next NewFrame() call.
// Clipboard Utilities
// - Also see the LogToClipboard() function to capture GUI into clipboard, or easily output text data to the clipboard.
@[c:'ImGui_GetClipboardText']
fn im_gui_get_clipboard_text() &i8

@[c:'ImGui_SetClipboardText']
fn im_gui_set_clipboard_text(text &i8)
...
fn im_gui_load_ini_settings_from_disk(ini_filename &i8)

// call after CreateContext() and before the first call to NewFrame(). NewFrame() automatically calls LoadIniSettingsFromDisk(io.IniFilename).
@[c:'ImGui_LoadIniSettingsFromMemory']
fn im_gui_load_ini_settings_from_memory(ini_data &i8, ini_size usize)

// call after CreateContext() and before the first call to NewFrame() to provide .ini data from your own data source.
@[c:'ImGui_SaveIniSettingsToDisk']
fn im_gui_save_ini_settings_to_disk(ini_filename &i8)

// this is automatically called (if io.IniFilename is not empty) a few seconds after any modification that should be reflected in the .ini file (and also by DestroyContext).
@[c:'ImGui_SaveIniSettingsToMemory']
fn im_gui_save_ini_settings_to_memory(out_ini_size &usize) &i8

// return a zero-terminated string with the .ini data which you can save by your own mean. call when io.WantSaveIniSettings is set, then save data by your own mean and clear io.WantSaveIniSettings.
// Debug Utilities
// - Your main debugging friend is the ShowMetricsWindow() function, which is also accessible from Demo->Tools->Metrics Debugger
@[c:'ImGui_DebugTextEncoding']
fn im_gui_debug_text_encoding(text &i8)

@[c:'ImGui_DebugFlashStyleColor']
fn im_gui_debug_flash_style_color(idx ImGuiCol)

@[c:'ImGui_DebugStartItemPicker']
fn im_gui_debug_start_item_picker()

@[c:'ImGui_DebugCheckVersionAndDataLayout']
fn im_gui_debug_check_version_and_data_layout(version_str &i8, sz_io usize, sz_style usize, sz_vec2 usize, sz_vec4 usize, sz_drawvert usize, sz_drawidx usize) bool

// This is called by IMGUI_CHECKVERSION() macro.
@[c2v_variadic]
@[c:'ImGui_DebugLog']
fn im_gui_debug_log(fmt &i8)

// Call via IMGUI_DEBUG_LOG() for maximum stripping in caller code!
@[c:'ImGui_DebugLogV']
fn im_gui_debug_log_v(fmt &i8, args va_list)
...
fn im_gui_set_allocator_functions(alloc_func ImGuiMemAllocFunc, free_func ImGuiMemFreeFunc, user_data voidptr)

@[c:'ImGui_GetAllocatorFunctions']
fn im_gui_get_allocator_functions(p_alloc_func &ImGuiMemAllocFunc, p_free_func &ImGuiMemFreeFunc, p_user_data &voidptr)

@[c:'ImGui_MemAlloc']
fn im_gui_mem_alloc(size usize) voidptr

@[c:'ImGui_MemFree']
fn im_gui_mem_free(ptr voidptr)
...
// Display a dot next to the title. When used in a tab/docking context, tab is selected when clicking the X + closure is not assumed (will wait for user to stop submitting the tab). Otherwise closure is assumed when pressing the X, so if you keep submitting the tab may reappear at end of tab bar.
	im_gui_window_flags_no_nav = im_gui_window_flags_no_nav_inputs | im_gui_window_flags_no_nav_focus
	im_gui_window_flags_no_decoration = im_gui_window_flags_no_title_bar | im_gui_window_flags_no_resize | im_gui_window_flags_no_scrollbar | im_gui_window_flags_no_collapse
	im_gui_window_flags_no_inputs = im_gui_window_flags_no_mouse_inputs | im_gui_window_flags_no_nav_inputs | im_gui_window_flags_no_nav_focus
// [Internal]
...
// Obsolete names
	im_gui_window_flags_nav_flattened = 1 << 29
// Obsoleted in 1.90.9: Use ImGuiChildFlags_NavFlattened in BeginChild() call.
	im_gui_window_flags_always_use_window_padding = 1 << 30
...
// Obsolete names
	im_gui_child_flags_border = im_gui_child_flags_borders
...
// true     // MenuItem()/Selectable() automatically close their parent popup window.
	im_gui_item_flags_allow_duplicate_id = 1 << 5
...
// Elide display / Alignment
	im_gui_input_text_flags_elide_left = 1 << 17
// When text doesn't fit, elide left side to ensure right side stays visible. Useful for path/filenames. Single-line only!
// Callback features
	im_gui_input_text_flags_callback_completion = 1 << 18
// Callback on pressing TAB (for completion handling)
	im_gui_input_text_flags_callback_history = 1 << 19
// Callback on pressing Up/Down arrows (for history handling)
	im_gui_input_text_flags_callback_always = 1 << 20
// Callback on each iteration. User code may query cursor position, modify text buffer.
	im_gui_input_text_flags_callback_char_filter = 1 << 21
// Callback on character inputs to replace or discard them. Modify 'EventChar' to replace or discard, or return 1 in callback to discard.
	im_gui_input_text_flags_callback_resize = 1 << 22
// Callback on buffer capacity changes request (beyond 'buf_size' parameter value), allowing the string to grow. Notify when the string wants to be resized (for string types which hold a cache of their Size). You will be provided a new BufSize in the callback and NEED to honor it. (see misc/cpp/imgui_stdlib.h for an example of using this)
	im_gui_input_text_flags_callback_edit = 1 << 23
...
// Extend hit box to the left-most and right-most edges (cover the indent area).
	im_gui_tree_node_flags_span_label_width = 1 << 13
...
// Frame will span all columns of its container table (label will still fit in current column)
	im_gui_tree_node_flags_label_span_all_columns = 1 << 15
// Label will span all columns of its container table
//ImGuiTreeNodeFlags_NoScrollOnOpen     = 1 << 16,  // FIXME: TODO: Disable automatic scroll on TreePop() if node got just open and contents is not visible
	im_gui_tree_node_flags_nav_left_jumps_back_here = 1 << 17
// (WIP) Nav: left direction may move to this TreeNode() from any of its child (items submitted between TreeNode and TreePop)
	im_gui_tree_node_flags_collapsing_header = im_gui_tree_node_flags_framed | im_gui_tree_node_flags_no_tree_push_on_open | im_gui_tree_node_flags_no_auto_open_on_log
	im_gui_tree_node_flags_allow_item_overlap = im_gui_tree_node_flags_allow_overlap
// Renamed in 1.89.7
	im_gui_tree_node_flags_span_text_width = im_gui_tree_node_flags_span_label_width
// Renamed in 1.90.7
...
// For IsPopupOpen(): search/test at any level of the popup stack (default test in the current level)
	im_gui_popup_flags_any_popup = im_gui_popup_flags_any_popup_id | im_gui_popup_flags_any_popup_level
}
...
// Make the item be displayed as if it is hovered
	im_gui_selectable_flags_dont_close_popups = im_gui_selectable_flags_no_auto_close_popups
// Renamed in 1.91.0
	im_gui_selectable_flags_allow_item_overlap = im_gui_selectable_flags_allow_overlap
// Renamed in 1.89.7
...
// Width dynamically calculated from preview contents
	im_gui_combo_flags_height_mask_ = im_gui_combo_flags_height_small | im_gui_combo_flags_height_regular | im_gui_combo_flags_height_large | im_gui_combo_flags_height_largest
}
...
// Add scroll buttons when tabs don't fit
	im_gui_tab_bar_flags_fitting_policy_mask_ = im_gui_tab_bar_flags_fitting_policy_resize_down | im_gui_tab_bar_flags_fitting_policy_scroll
	im_gui_tab_bar_flags_fitting_policy_default_ = im_gui_tab_bar_flags_fitting_policy_resize_down
}
...
//ImGuiFocusedFlags_DockHierarchy               = 1 << 4,   // Consider docking hierarchy (treat dockspace host as parent of docked window) (when used with _ChildWindows or _RootWindow)
	im_gui_focused_flags_root_and_child_windows = im_gui_focused_flags_root_window | im_gui_focused_flags_child_windows
}
...
// IsItemHovered() only: Disable using keyboard/gamepad navigation state when active, always query mouse
	im_gui_hovered_flags_allow_when_overlapped = im_gui_hovered_flags_allow_when_overlapped_by_item | im_gui_hovered_flags_allow_when_overlapped_by_window
	im_gui_hovered_flags_rect_only = im_gui_hovered_flags_allow_when_blocked_by_popup | im_gui_hovered_flags_allow_when_blocked_by_active_item | im_gui_hovered_flags_allow_when_overlapped
	im_gui_hovered_flags_root_and_child_windows = im_gui_hovered_flags_root_window | im_gui_hovered_flags_child_windows
// Tooltips mode
...
// Request hiding the BeginDragDropSource tooltip from the BeginDragDropTarget site.
	im_gui_drag_drop_flags_accept_peek_only = im_gui_drag_drop_flags_accept_before_delivery | im_gui_drag_drop_flags_accept_no_draw_default_rect
// For peeking ahead and inspecting the payload before delivery.
	im_gui_drag_drop_flags_source_auto_expire_payload = im_gui_drag_drop_flags_payload_auto_expire
// Renamed in 1.90.9
...
// == ImGuiKey_NamedKey_BEGIN
	im_gui_key_left_arrow = 3
	im_gui_key_right_arrow = 4
	im_gui_key_up_arrow = 5
	im_gui_key_down_arrow = 6
	im_gui_key_page_up = 7
	im_gui_key_page_down = 8
	im_gui_key_home = 9
	im_gui_key_end = 10
	im_gui_key_insert = 11
	im_gui_key_delete = 12
	im_gui_key_backspace = 13
	im_gui_key_space = 14
	im_gui_key_enter = 15
	im_gui_key_escape = 16
	im_gui_key_left_ctrl = 17
	im_gui_key_left_shift = 18
	im_gui_key_left_alt = 19
	im_gui_key_left_super = 20
	im_gui_key_right_ctrl = 21
	im_gui_key_right_shift = 22
	im_gui_key_right_alt = 23
	im_gui_key_right_super = 24
	im_gui_key_menu = 25
	im_gui_key_0 = 26
	im_gui_key_1 = 27
	im_gui_key_2 = 28
	...
	im_gui_key_f23 = 84
	im_gui_key_f24 = 85
	im_gui_key_apostrophe = 86
// '
	im_gui_key_comma = 87
// ,
	im_gui_key_minus = 88
// -
	im_gui_key_period = 89
// .
	im_gui_key_slash = 90
// /
	im_gui_key_semicolon = 91
// ;
	im_gui_key_equal = 92
// =
	im_gui_key_left_bracket = 93
// [
	im_gui_key_backslash = 94
// \ (this text inhibit multiline comment caused by backslash)
	im_gui_key_right_bracket = 95
// ]
	im_gui_key_grave_accent = 96
// `
	im_gui_key_caps_lock = 97
	im_gui_key_scroll_lock = 98
	im_gui_key_num_lock = 99
	im_gui_key_print_screen = 100
	im_gui_key_pause = 101
	im_gui_key_keypad0 = 102
	im_gui_key_keypad1 = 103
	im_gui_key_keypad2 = 104
	im_gui_key_keypad3 = 105
	im_gui_key_keypad4 = 106
	im_gui_key_keypad5 = 107
	im_gui_key_keypad6 = 108
	im_gui_key_keypad7 = 109
	im_gui_key_keypad8 = 110
	im_gui_key_keypad9 = 111
	im_gui_key_keypad_decimal = 112
	im_gui_key_keypad_divide = 113
	im_gui_key_keypad_multiply = 114
	im_gui_key_keypad_subtract = 115
	im_gui_key_keypad_add = 116
	im_gui_key_keypad_enter = 117
	im_gui_key_keypad_equal = 118
	im_gui_key_app_back = 119
// Available on some keyboard/mouses. Often referred as "Browser Back"
	im_gui_key_app_forward = 120
// Gamepad (some of those are analog values, 0.0f to 1.0f)                          // NAVIGATION ACTION
// (download controller mapping PNG/PSD at http://dearimgui.com/controls_sheets)
	im_gui_key_gamepad_start = 121
// Menu (Xbox)      + (Switch)   Start/Options (PS)
	im_gui_key_gamepad_back = 122
// View (Xbox)      - (Switch)   Share (PS)
	im_gui_key_gamepad_face_left = 123
// X (Xbox)         Y (Switch)   Square (PS)        // Tap: Toggle Menu. Hold: Windowing mode (Focus/Move/Resize windows)
	im_gui_key_gamepad_face_right = 124
// B (Xbox)         A (Switch)   Circle (PS)        // Cancel / Close / Exit
	im_gui_key_gamepad_face_up = 125
// Y (Xbox)         X (Switch)   Triangle (PS)      // Text Input / On-screen Keyboard
	im_gui_key_gamepad_face_down = 126
// A (Xbox)         B (Switch)   Cross (PS)         // Activate / Open / Toggle / Tweak
	im_gui_key_gamepad_dpad_left = 127
// D-pad Left                                       // Move / Tweak / Resize Window (in Windowing mode)
	im_gui_key_gamepad_dpad_right = 128
// D-pad Right                                      // Move / Tweak / Resize Window (in Windowing mode)
	im_gui_key_gamepad_dpad_up = 129
// D-pad Up                                         // Move / Tweak / Resize Window (in Windowing mode)
	im_gui_key_gamepad_dpad_down = 130
// D-pad Down                                       // Move / Tweak / Resize Window (in Windowing mode)
	im_gui_key_gamepad_l1 = 131
// L Bumper (Xbox)  L (Switch)   L1 (PS)            // Tweak Slower / Focus Previous (in Windowing mode)
	im_gui_key_gamepad_r1 = 132
// R Bumper (Xbox)  R (Switch)   R1 (PS)            // Tweak Faster / Focus Next (in Windowing mode)
	im_gui_key_gamepad_l2 = 133
// L Trig. (Xbox)   ZL (Switch)  L2 (PS) [Analog]
	im_gui_key_gamepad_r2 = 134
// R Trig. (Xbox)   ZR (Switch)  R2 (PS) [Analog]
	im_gui_key_gamepad_l3 = 135
// L Stick (Xbox)   L3 (Switch)  L3 (PS)
	im_gui_key_gamepad_r3 = 136
// R Stick (Xbox)   R3 (Switch)  R3 (PS)
	im_gui_key_gamepad_ls_tick_left = 137
// [Analog]                                         // Move Window (in Windowing mode)
	im_gui_key_gamepad_ls_tick_right = 138
// [Analog]                                         // Move Window (in Windowing mode)
	im_gui_key_gamepad_ls_tick_up = 139
// [Analog]                                         // Move Window (in Windowing mode)
	im_gui_key_gamepad_ls_tick_down = 140
// [Analog]                                         // Move Window (in Windowing mode)
	im_gui_key_gamepad_rs_tick_left = 141
// [Analog]
	im_gui_key_gamepad_rs_tick_right = 142
// [Analog]
	im_gui_key_gamepad_rs_tick_up = 143
// [Analog]
	im_gui_key_gamepad_rs_tick_down = 144
// [Analog]
// Aliases: Mouse Buttons (auto-submitted from AddMouseButtonEvent() calls)
// - This is mirroring the data also written to io.MouseDown[], io.MouseWheel, in a format allowing them to be accessed via standard key API.
	im_gui_key_mouse_left = 145
	im_gui_key_mouse_right = 146
	im_gui_key_mouse_middle = 147
	im_gui_key_mouse_x1 = 148
	im_gui_key_mouse_x2 = 149
	im_gui_key_mouse_wheel_x = 150
	im_gui_key_mouse_wheel_y = 151
// [Internal] Reserved for mod storage
	im_gui_key_reserved_for_mod_ctrl = 152
	im_gui_key_reserved_for_mod_shift = 153
	im_gui_key_reserved_for_mod_alt = 154
	im_gui_key_reserved_for_mod_super = 155
	im_gui_key_named_key_end = 156
...
// [Internal] If you need to iterate all keys (for e.g. an input mapper) you may use ImGuiKey_NamedKey_BEGIN..ImGuiKey_NamedKey_END.
	im_gui_key_named_key_count = im_gui_key_named_key_end - im_gui_key_named_key_begin
...
//ImGuiKey_KeysData_OFFSET      = ImGuiKey_NamedKey_BEGIN,  // Accesses to io.KeysData[] must use (key - ImGuiKey_NamedKey_BEGIN) index.
	im_gui_key_count = im_gui_key_named_key_end
// Obsoleted in 1.91.5 because it was extremely misleading (since named keys don't start at 0 anymore)
	im_gui_mod_shortcut = im_gui_mod_ctrl
// Removed in 1.90.7, you can now simply use ImGuiMod_Ctrl
	im_gui_key_mod_ctrl = im_gui_mod_ctrl
	im_gui_key_mod_shift = im_gui_mod_shift
	im_gui_key_mod_alt = im_gui_mod_alt
	im_gui_key_mod_super = im_gui_mod_super
// Renamed in 1.89
...
// Darken/colorize entire screen behind a modal window, when one is active
	im_gui_col_count
	im_gui_col_tab_active = im_gui_col_tab_selected
// [renamed in 1.90.9]
	im_gui_col_tab_unfocused = im_gui_col_tab_dimmed
// [renamed in 1.90.9]
	im_gui_col_tab_unfocused_active = im_gui_col_tab_dimmed_selected
// [renamed in 1.90.9]
	im_gui_col_nav_highlight = im_gui_col_nav_cursor
// [renamed in 1.91.4]
...
// float     GrabRounding
	im_gui_style_var_image_border_size
...
// React on center mouse button
	im_gui_button_flags_mouse_button_mask_ = im_gui_button_flags_mouse_button_left | im_gui_button_flags_mouse_button_right | im_gui_button_flags_mouse_button_middle
// [Internal]
	im_gui_button_flags_enable_nav = 1 << 3
...
// - The new flags may be combined better and allow finer controls.
	im_gui_color_edit_flags_alpha_opaque = 1 << 11
...
// override them in most of your calls. Let the user choose via the option menu and/or call SetColorEditOptions() once during startup.
	im_gui_color_edit_flags_default_options_ = im_gui_color_edit_flags_uint8 | im_gui_color_edit_flags_display_rgb | im_gui_color_edit_flags_input_rgb | im_gui_color_edit_flags_picker_hue_bar
// [Internal] Masks
	im_gui_color_edit_flags_alpha_mask_ = im_gui_color_edit_flags_no_alpha | im_gui_color_edit_flags_alpha_opaque | im_gui_color_edit_flags_alpha_no_bg | im_gui_color_edit_flags_alpha_preview_half
	im_gui_color_edit_flags_display_mask_ = im_gui_color_edit_flags_display_rgb | im_gui_color_edit_flags_display_hsv | im_gui_color_edit_flags_display_hex
...
// Obsolete names
	im_gui_color_edit_flags_alpha_preview = 0
...
// Enable wrapping around from max to min and from min to max. Only supported by DragXXX() functions for now.
	im_gui_slider_flags_clamp_on_input = 1 << 9
// Clamp value to min/max bounds when input manually with CTRL+Click. By default CTRL+Click allows going out of bounds.
	im_gui_slider_flags_clamp_zero_range = 1 << 10
// Clamp even if min==max==0.0f. Otherwise due to legacy reason DragXXX functions don't clamp with those values. When your clamping limits are dynamic you almost always want to use it.
	im_gui_slider_flags_no_speed_tweaks = 1 << 11
// Disable keyboard modifiers altering tweak speed. Useful if you want to alter tweak speed yourself based on your own logic.
	im_gui_slider_flags_always_clamp = im_gui_slider_flags_clamp_on_input | im_gui_slider_flags_clamp_zero_range
...
// (Unused by Dear ImGui functions. Use for e.g. hyperlinks)
	im_gui_mouse_cursor_wait
// When waiting for something to process/load.
	im_gui_mouse_cursor_progress
...
fn im_gui_style_scale_all_sizes(self &ImGuiStyle, scale_factor f32)
...
fn im_gui_io_add_key_event(self &ImGuiIO, key ImGuiKey, down bool)

// Queue a new key down/up event. Key should be "translated" (as in, generally ImGuiKey_A matches the key end-user would use to emit an 'A' character)
@[c:'ImGuiIO_AddKeyAnalogEvent']
fn im_gui_io_add_key_analog_event(self &ImGuiIO, key ImGuiKey, down bool, v f32)

// Queue a new key down/up event for analog values (e.g. ImGuiKey_Gamepad_ values). Dead-zones should be handled by the backend.
@[c:'ImGuiIO_AddMousePosEvent']
fn im_gui_io_add_mouse_pos_event(self &ImGuiIO, x f32, y f32)

// Queue a mouse position update. Use -FLT_MAX,-FLT_MAX to signify no mouse (e.g. app not focused and not hovered)
@[c:'ImGuiIO_AddMouseButtonEvent']
fn im_gui_io_add_mouse_button_event(self &ImGuiIO, button int, down bool)

// Queue a mouse button change
@[c:'ImGuiIO_AddMouseWheelEvent']
fn im_gui_io_add_mouse_wheel_event(self &ImGuiIO, wheel_x f32, wheel_y f32)

// Queue a mouse wheel update. wheel_y<0: scroll down, wheel_y>0: scroll up, wheel_x<0: scroll right, wheel_x>0: scroll left.
@[c:'ImGuiIO_AddMouseSourceEvent']
fn im_gui_io_add_mouse_source_event(self &ImGuiIO, source ImGuiMouseSource)

// Queue a mouse source change (Mouse/TouchScreen/Pen)
@[c:'ImGuiIO_AddFocusEvent']
fn im_gui_io_add_focus_event(self &ImGuiIO, focused bool)

// Queue a gain/loss of focus for the application (generally based on OS/platform focus of your window)
@[c:'ImGuiIO_AddInputCharacter']
fn im_gui_io_add_input_character(self &ImGuiIO, c u32)

// Queue a new character input
@[c:'ImGuiIO_AddInputCharacterUTF16']
fn im_gui_io_add_input_character_utf_16(self &ImGuiIO, c ImWchar16)

// Queue a new character input from a UTF-16 character, it can be a surrogate
@[c:'ImGuiIO_AddInputCharactersUTF8']
fn im_gui_io_add_input_characters_utf_8(self &ImGuiIO, str &i8)

// Queue a new characters input from a UTF-8 string
@[c:'ImGuiIO_SetKeyEventNativeData']
fn im_gui_io_set_key_event_native_data(self &ImGuiIO, key ImGuiKey, native_keycode int, native_scancode int)

// Implied native_legacy_index = -1
@[c:'ImGuiIO_SetKeyEventNativeDataEx']
fn im_gui_io_set_key_event_native_data_ex(self &ImGuiIO, key ImGuiKey, native_keycode int, native_scancode int, native_legacy_index int)

// [Optional] Specify index for legacy <1.87 IsKeyXXX() functions with native indices + specify native keycode, scancode.
@[c:'ImGuiIO_SetAppAcceptingEvents']
fn im_gui_io_set_app_accepting_events(self &ImGuiIO, accepting_events bool)

// Set master flag for accepting key/mouse/text events (default to true). Useful if you have native dialog boxes that are interrupting your application loop/refresh, and you want to disable events being queued while your app is frozen.
@[c:'ImGuiIO_ClearEventsQueue']
fn im_gui_io_clear_events_queue(self &ImGuiIO)

// Clear all incoming events.
@[c:'ImGuiIO_ClearInputKeys']
fn im_gui_io_clear_input_keys(self &ImGuiIO)

// Clear current keyboard/gamepad state + current frame text input buffer. Equivalent to releasing all keys/buttons.
@[c:'ImGuiIO_ClearInputMouse']
fn im_gui_io_clear_input_mouse(self &ImGuiIO)

// Clear current mouse state.
@[c:'ImGuiIO_ClearInputCharacters']
fn im_gui_io_clear_input_characters(self &ImGuiIO)
...
fn im_gui_input_text_callback_data_delete_chars(self &ImGuiInputTextCallbackData, pos int, bytes_count int)

@[c:'ImGuiInputTextCallbackData_InsertChars']
fn im_gui_input_text_callback_data_insert_chars(self &ImGuiInputTextCallbackData, pos int, text &i8, text_end &i8)

@[c:'ImGuiInputTextCallbackData_SelectAll']
fn im_gui_input_text_callback_data_select_all(self &ImGuiInputTextCallbackData)

@[c:'ImGuiInputTextCallbackData_ClearSelection']
fn im_gui_input_text_callback_data_clear_selection(self &ImGuiInputTextCallbackData)

@[c:'ImGuiInputTextCallbackData_HasSelection']
fn im_gui_input_text_callback_data_has_selection(self &ImGuiInputTextCallbackData) bool
...
fn im_gui_payload_clear(self &ImGuiPayload)

@[c:'ImGuiPayload_IsDataType']
fn im_gui_payload_is_data_type(self &ImGuiPayload, type_ &i8) bool

@[c:'ImGuiPayload_IsPreview']
fn im_gui_payload_is_preview(self &ImGuiPayload) bool

@[c:'ImGuiPayload_IsDelivery']
fn im_gui_payload_is_delivery(self &ImGuiPayload) bool
...
fn im_gui_text_filter_im_gui_text_range_empty(self &ImGuiTextFilter_ImGuiTextRange) bool

@[c:'ImGuiTextFilter_ImGuiTextRange_split']
fn im_gui_text_filter_im_gui_text_range_split(self &ImGuiTextFilter_ImGuiTextRange, separator i8, out &ImVector_ImGuiTextFilter_ImGuiTextRange)
...
fn im_gui_text_filter_draw(self &ImGuiTextFilter, label &i8, width f32) bool

// Helper calling InputText+Build
@[c:'ImGuiTextFilter_PassFilter']
fn im_gui_text_filter_pass_filter(self &ImGuiTextFilter, text &i8, text_end &i8) bool

@[c:'ImGuiTextFilter_Build']
fn im_gui_text_filter_build(self &ImGuiTextFilter)

@[c:'ImGuiTextFilter_Clear']
fn im_gui_text_filter_clear(self &ImGuiTextFilter)

@[c:'ImGuiTextFilter_IsActive']
fn im_gui_text_filter_is_active(self &ImGuiTextFilter) bool
...
fn im_gui_text_buffer_begin(self &ImGuiTextBuffer) &i8

@[c:'ImGuiTextBuffer_end']
fn im_gui_text_buffer_end(self &ImGuiTextBuffer) &i8

// Buf is zero-terminated, so end() will point on the zero-terminator
@[c:'ImGuiTextBuffer_size']
fn im_gui_text_buffer_size(self &ImGuiTextBuffer) int

@[c:'ImGuiTextBuffer_empty']
fn im_gui_text_buffer_empty(self &ImGuiTextBuffer) bool

@[c:'ImGuiTextBuffer_clear']
fn im_gui_text_buffer_clear(self &ImGuiTextBuffer)

@[c:'ImGuiTextBuffer_resize']
fn im_gui_text_buffer_resize(self &ImGuiTextBuffer, size int)

// Similar to resize(0) on ImVector: empty string but don't free buffer.
@[c:'ImGuiTextBuffer_reserve']
fn im_gui_text_buffer_reserve(self &ImGuiTextBuffer, capacity int)

@[c:'ImGuiTextBuffer_c_str']
fn im_gui_text_buffer_c_str(self &ImGuiTextBuffer) &i8

@[c:'ImGuiTextBuffer_append']
fn im_gui_text_buffer_append(self &ImGuiTextBuffer, str &i8, str_end &i8)

@[c2v_variadic]
@[c:'ImGuiTextBuffer_appendf']
fn im_gui_text_buffer_appendf(self &ImGuiTextBuffer, fmt &i8)

@[c:'ImGuiTextBuffer_appendfv']
fn im_gui_text_buffer_appendfv(self &ImGuiTextBuffer, fmt &i8, args va_list)
...
fn im_gui_storage_clear(self &ImGuiStorage)

@[c:'ImGuiStorage_GetInt']
fn im_gui_storage_get_int(self &ImGuiStorage, key ImGuiID, default_val int) int

@[c:'ImGuiStorage_SetInt']
fn im_gui_storage_set_int(self &ImGuiStorage, key ImGuiID, val int)

@[c:'ImGuiStorage_GetBool']
fn im_gui_storage_get_bool(self &ImGuiStorage, key ImGuiID, default_val bool) bool

@[c:'ImGuiStorage_SetBool']
fn im_gui_storage_set_bool(self &ImGuiStorage, key ImGuiID, val bool)

@[c:'ImGuiStorage_GetFloat']
fn im_gui_storage_get_float(self &ImGuiStorage, key ImGuiID, default_val f32) f32

@[c:'ImGuiStorage_SetFloat']
fn im_gui_storage_set_float(self &ImGuiStorage, key ImGuiID, val f32)

@[c:'ImGuiStorage_GetVoidPtr']
fn im_gui_storage_get_void_ptr(self &ImGuiStorage, key ImGuiID) voidptr

// default_val is NULL
@[c:'ImGuiStorage_SetVoidPtr']
fn im_gui_storage_set_void_ptr(self &ImGuiStorage, key ImGuiID, val voidptr)
...
fn im_gui_storage_get_int_ref(self &ImGuiStorage, key ImGuiID, default_val int) &int

@[c:'ImGuiStorage_GetBoolRef']
fn im_gui_storage_get_bool_ref(self &ImGuiStorage, key ImGuiID, default_val bool) &bool

@[c:'ImGuiStorage_GetFloatRef']
fn im_gui_storage_get_float_ref(self &ImGuiStorage, key ImGuiID, default_val f32) &f32

@[c:'ImGuiStorage_GetVoidPtrRef']
fn im_gui_storage_get_void_ptr_ref(self &ImGuiStorage, key ImGuiID, default_val voidptr) &voidptr

// Advanced: for quicker full rebuild of a storage (instead of an incremental one), you may add all your contents and then sort once.
@[c:'ImGuiStorage_BuildSortByKey']
fn im_gui_storage_build_sort_by_key(self &ImGuiStorage)

// Obsolete: use on your own storage if you know only integer are being stored (open/close all tree nodes)
@[c:'ImGuiStorage_SetAllInt']
fn im_gui_storage_set_all_int(self &ImGuiStorage, val int)
...
fn im_gui_list_clipper_begin(self &ImGuiListClipper, items_count int, items_height f32)

@[c:'ImGuiListClipper_End']
fn im_gui_list_clipper_end(self &ImGuiListClipper)

// Automatically called on the last call of Step() that returns false.
@[c:'ImGuiListClipper_Step']
fn im_gui_list_clipper_step(self &ImGuiListClipper) bool

// Call until it returns false. The DisplayStart/DisplayEnd fields will be set and you can process/draw those items.
// Call IncludeItemByIndex() or IncludeItemsByIndex() *BEFORE* first call to Step() if you need a range of items to not be clipped, regardless of their visibility.
// (Due to alignment / padding of certain items it is possible that an extra item may be included on either end of the display range).
@[c:'ImGuiListClipper_IncludeItemByIndex']
fn im_gui_list_clipper_include_item_by_index(self &ImGuiListClipper, item_index int)

@[c:'ImGuiListClipper_IncludeItemsByIndex']
fn im_gui_list_clipper_include_items_by_index(self &ImGuiListClipper, item_begin int, item_end int)
...
fn im_gui_list_clipper_seek_cursor_for_item(self &ImGuiListClipper, item_index int)

@[c:'ImGuiListClipper_IncludeRangeByIndices']
fn im_gui_list_clipper_include_range_by_indices(self &ImGuiListClipper, item_begin int, item_end int)

// [renamed in 1.89.9]
@[c:'ImGuiListClipper_ForceDisplayRangeByIndices']
fn im_gui_list_clipper_force_display_range_by_indices(self &ImGuiListClipper, item_begin int, item_end int)
...
fn im_gui_selection_basic_storage_apply_requests(self &ImGuiSelectionBasicStorage, ms_io &ImGuiMultiSelectIO)
...
fn im_gui_selection_basic_storage_contains(self &ImGuiSelectionBasicStorage, id ImGuiID) bool

// Query if an item id is in selection.
@[c:'ImGuiSelectionBasicStorage_Clear']
fn im_gui_selection_basic_storage_clear(self &ImGuiSelectionBasicStorage)

// Clear selection
@[c:'ImGuiSelectionBasicStorage_Swap']
fn im_gui_selection_basic_storage_swap(self &ImGuiSelectionBasicStorage, r &ImGuiSelectionBasicStorage)

// Swap two selections
@[c:'ImGuiSelectionBasicStorage_SetItemSelected']
fn im_gui_selection_basic_storage_set_item_selected(self &ImGuiSelectionBasicStorage, id ImGuiID, selected bool)
...
fn im_gui_selection_basic_storage_get_next_selected_item(self &ImGuiSelectionBasicStorage, opaque_it &voidptr, out_id &ImGuiID) bool

// Iterate selection with 'void* it = NULL; ImGuiID id; while (selection.GetNextSelectedItem(&it, &id)) { ... }'
@[c:'ImGuiSelectionBasicStorage_GetStorageIdFromIndex']
fn im_gui_selection_basic_storage_get_storage_id_from_index(self &ImGuiSelectionBasicStorage, idx int) ImGuiID
...
fn im_gui_selection_external_storage_apply_requests(self &ImGuiSelectionExternalStorage, ms_io &ImGuiMultiSelectIO)
...
fn im_font_glyph_ranges_builder_add_char(self &ImFontGlyphRangesBuilder, c ImWchar)
...
fn im_font_glyph_ranges_builder_add_ranges(self &ImFontGlyphRangesBuilder, ranges &ImWchar)
...
fn im_font_atlas_add_font_from_file_ttf(self &ImFontAtlas, filename &i8, size_pixels f32, font_cfg &ImFontConfig, glyph_ranges &ImWchar) &ImFont

@[c:'ImFontAtlas_AddFontFromMemoryTTF']
fn im_font_atlas_add_font_from_memory_ttf(self &ImFontAtlas, font_data voidptr, font_data_size int, size_pixels f32, font_cfg &ImFontConfig, glyph_ranges &ImWchar) &ImFont

// Note: Transfer ownership of 'ttf_data' to ImFontAtlas! Will be deleted after destruction of the atlas. Set font_cfg->FontDataOwnedByAtlas=false to keep ownership of your data and it won't be freed.
@[c:'ImFontAtlas_AddFontFromMemoryCompressedTTF']
fn im_font_atlas_add_font_from_memory_compressed_ttf(self &ImFontAtlas, compressed_font_data voidptr, compressed_font_data_size int, size_pixels f32, font_cfg &ImFontConfig, glyph_ranges &ImWchar) &ImFont

// 'compressed_font_data' still owned by caller. Compress with binary_to_compressed_c.cpp.
@[c:'ImFontAtlas_AddFontFromMemoryCompressedBase85TTF']
fn im_font_atlas_add_font_from_memory_compressed_base85_ttf(self &ImFontAtlas, compressed_font_data_base85 &i8, size_pixels f32, font_cfg &ImFontConfig, glyph_ranges &ImWchar) &ImFont
...
fn im_font_atlas_get_glyph_ranges_default(self &ImFontAtlas) &ImWchar

// Basic Latin, Extended Latin
@[c:'ImFontAtlas_GetGlyphRangesGreek']
fn im_font_atlas_get_glyph_ranges_greek(self &ImFontAtlas) &ImWchar

// Default + Greek and Coptic
@[c:'ImFontAtlas_GetGlyphRangesKorean']
fn im_font_atlas_get_glyph_ranges_korean(self &ImFontAtlas) &ImWchar

// Default + Korean characters
@[c:'ImFontAtlas_GetGlyphRangesJapanese']
fn im_font_atlas_get_glyph_ranges_japanese(self &ImFontAtlas) &ImWchar

// Default + Hiragana, Katakana, Half-Width, Selection of 2999 Ideographs
@[c:'ImFontAtlas_GetGlyphRangesChineseFull']
fn im_font_atlas_get_glyph_ranges_chinese_full(self &ImFontAtlas) &ImWchar

// Default + Half-Width + Japanese Hiragana/Katakana + full set of about 21000 CJK Unified Ideographs
@[c:'ImFontAtlas_GetGlyphRangesChineseSimplifiedCommon']
fn im_font_atlas_get_glyph_ranges_chinese_simplified_common(self &ImFontAtlas) &ImWchar

// Default + Half-Width + Japanese Hiragana/Katakana + set of 2500 CJK Unified Ideographs for common simplified Chinese
@[c:'ImFontAtlas_GetGlyphRangesCyrillic']
fn im_font_atlas_get_glyph_ranges_cyrillic(self &ImFontAtlas) &ImWchar

// Default + about 400 Cyrillic characters
@[c:'ImFontAtlas_GetGlyphRangesThai']
fn im_font_atlas_get_glyph_ranges_thai(self &ImFontAtlas) &ImWchar

// Default + Thai characters
@[c:'ImFontAtlas_GetGlyphRangesVietnamese']
fn im_font_atlas_get_glyph_ranges_vietnamese(self &ImFontAtlas) &ImWchar
...
fn im_font_atlas_add_custom_rect_font_glyph(self &ImFontAtlas, font &ImFont, id ImWchar, width int, height int, advance_x f32, offset ImVec2) int
...
@[c:'ImFont_FindGlyph']
fn im_font_find_glyph(self &ImFont, c ImWchar) &ImFontGlyph

@[c:'ImFont_FindGlyphNoFallback']
fn im_font_find_glyph_no_fallback(self &ImFont, c ImWchar) &ImFontGlyph

@[c:'ImFont_GetCharAdvance']
fn im_font_get_char_advance(self &ImFont, c ImWchar) f32
...
@[c:'ImFont_RenderChar']
fn im_font_render_char(self &ImFont, draw_list &ImDrawList, size f32, pos ImVec2, col ImU32, c ImWchar)
...
@[c:'ImFont_AddGlyph']
fn im_font_add_glyph(self &ImFont, src_cfg &ImFontConfig, c ImWchar, x0 f32, y0 f32, x1 f32, y1 f32, u0 f32, v0 f32, u1 f32, v1 f32, advance_x f32)

@[c:'ImFont_AddRemapChar']
fn im_font_add_remap_char(self &ImFont, dst ImWchar, src ImWchar, overwrite_dst bool)
...
fn im_gui_viewport_get_center(self &ImGuiViewport) ImVec2

@[c:'ImGuiViewport_GetWorkCenter']
fn im_gui_viewport_get_work_center(self &ImGuiViewport) ImVec2
...
@[c:'ImGui_ImageImVec4']
fn im_gui_image_im_vec4(user_texture_id ImTextureID, image_size ImVec2, uv0 ImVec2, uv1 ImVec2, tint_col ImVec4, border_col ImVec4)

// <-- border_col was removed in favor of ImGuiCol_ImageBorder.
// OBSOLETED in 1.91.0 (from July 2024)
@[c:'ImGui_PushButtonRepeat']
fn im_gui_push_button_repeat(repeat bool)

@[c:'ImGui_PopButtonRepeat']
fn im_gui_pop_button_repeat()

@[c:'ImGui_PushTabStop']
fn im_gui_push_tab_stop(tab_stop bool)

@[c:'ImGui_PopTabStop']
fn im_gui_pop_tab_stop()

@[c:'ImGui_GetContentRegionMax']
fn im_gui_get_content_region_max() ImVec2
...
fn im_gui_get_window_content_region_min() ImVec2
...
fn im_gui_get_window_content_region_max() ImVec2
...
fn im_gui_begin_child_frame(id ImGuiID, size ImVec2) bool

// Implied window_flags = 0
@[c:'ImGui_BeginChildFrameEx']
fn im_gui_begin_child_frame_ex(id ImGuiID, size ImVec2, window_flags ImGuiWindowFlags) bool

@[c:'ImGui_EndChildFrame']
fn im_gui_end_child_frame()
...
fn im_gui_show_stack_tool_window(p_open &bool)

@[c:'ImGui_ComboObsolete']
fn im_gui_combo_obsolete(label &i8, current_item &int, old_callback fn (voidptr, int, &&i8) bool, user_data voidptr, items_count int) bool

// Implied popup_max_height_in_items = -1
@[c:'ImGui_ComboObsoleteEx']
fn im_gui_combo_obsolete_ex(label &i8, current_item &int, old_callback fn (voidptr, int, &&i8) bool, user_data voidptr, items_count int, popup_max_height_in_items int) bool

@[c:'ImGui_ListBoxObsolete']
fn im_gui_list_box_obsolete(label &i8, current_item &int, old_callback fn (voidptr, int, &&i8) bool, user_data voidptr, items_count int) bool

// Implied height_in_items = -1
@[c:'ImGui_ListBoxObsoleteEx']
fn im_gui_list_box_obsolete_ex(label &i8, current_item &int, old_callback fn (voidptr, int, &&i8) bool, user_data voidptr, items_count int, height_in_items int) bool

// OBSOLETED in 1.89.7 (from June 2023)
@[c:'ImGui_SetItemAllowOverlap']
fn im_gui_set_item_allow_overlap()

// Use SetNextItemAllowOverlap() before item.
// OBSOLETED in 1.89.4 (from March 2023)
@[c:'ImGui_PushAllowKeyboardFocus']
fn im_gui_push_allow_keyboard_focus(tab_stop bool)

@[c:'ImGui_PopAllowKeyboardFocus']
fn im_gui_pop_allow_keyboard_focus()
